	...
	if ($_POST['format']=='xls')
	{
		...
		foreach ($outcash['link'] as $key => $llink)
		{
			...
			if (($nick!='') && ($hn!='.'))
			{
				...
				if ($hn=='mail.ru')
				{
					$top_speakers[$nick]['link']='http://blogs.'.$outcash['blog_link'][$key].'/'.$login;
				}
			}
		...
		if (is_resource($process))
		{
			...
			header("Content-Disposition: attachment; filename=wobot_".date('dmy',strtotime($_POST['stime'])).'_'.date('dmy',strtotime($_POST['etime'])).'_'.$file_name.".xls");
			echo $fulltext;
