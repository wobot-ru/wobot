...
if ($_POST['contact']!='')
{
	...
	$_POST['contact']=preg_replace('/[^\d\-\+\/\s]/isu', '', $_POST['contact']);
	...
	if (check_pass($_POST['pass']) && check_pass($_POST['ver_pass']))
	{
		...
		else
		{
			...
			if (preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/is',trim($user['user_email'])))
			{
				...
				$text_message = "Ваш пароль был изменен, для подтверждения пройдите по <a href='http://production.wobot.ru/accept_pass.php?token=".md5($_POST['pass'])."&uid=".$user['user_id']."&lst=".$user['user_pass']."'>ссылке</a><div><i><img src='http://wobot.ru/new/assets/logo.png'></i></div>";
				parseUrlmail('http://188.120.239.225/api/service/sendmail.php',$user['user_email'],'Команда Wobot',urlencode($text_message),$headers);
				$outmas['status']=6;
				echo json_encode($outmas);
				//die();
				//print_r($_COOKIE);
	...
	foreach ($mails as $mail)
	{
		if (preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/is',trim($mail)))
		{
			if (!in_array($mail,$us_mails))
			{
				...
				$text_message = "Вас добавили в список рассылки, пожалуйста для подтверждения пройдите по ссылке <a href='http://wobot.ru/accept_dgst.php?token=".md5(trim($mail))."&server=".$config_server['server_ip']."'>ссылка</a><div><i><img src='http://wobot.ru/new/assets/logo.png'></i></div>";
				parseUrlmail('http://188.120.239.225/api/service/sendmail.php',$mail,'Команда Wobot',urlencode($text_message),$headers);
			}
			else
			{
				$mails_upd[]=trim($mail);
...
$memcache->delete('user_session_'.session_id());
