...
if (($_POST['order_id']!='') && ($_POST['subtheme_id']!='') && ($_POST['subtheme_name']!=''))
{
	...
	$qsth=$db->query('SELECT * FROM blog_subthemes WHERE order_id='.intval($_POST['order_id']).' AND subtheme_id=\''.addslashes($_POST['subtheme_id']).'\'');
	if ($db->num_rows($qsth)!=0)
	{
		...
		//echo 'UPDATE blog_subthemes SET subtheme_name=\''.addslashes($_POST['subtheme_name']).'\',subtheme_settings=\''.addslashes(json_encode($settings)).'\' WHERE order_id='.intval($_POST['order_id']).' AND subtheme_id='.intval($_POST['subtheme_id']);
		$db->query('UPDATE blog_subthemes SET subtheme_name=\''.addslashes($_POST['subtheme_name']).'\',subtheme_settings=\''.addslashes(json_encode($settings)).'\' WHERE order_id='.intval($_POST['order_id']).' AND subtheme_id='.intval($_POST['subtheme_id']));
