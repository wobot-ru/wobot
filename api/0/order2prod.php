	...
	if ((strtotime($_GET['start'])!=0) && (strtotime($_GET['end'])!=0))
	{
		...
		for($t=strtotime($_GET['start']);$t<=strtotime($_GET['end']);$t=mktime(0,0,0,date("n",$t),date("j",$t)+1,date("Y",$t)))
		{
			...
			foreach ($m_dinams['sp_pr'][$t] as $key => $item)
			{
				...
				{
					//echo '...'.$iii['login'].' '.$key."\n";
					if ((trim($key)=='') || (trim($iii['login'])=='')) continue;
					if (!isset($y_prms[mb_strtolower($key,'UTF-8').':'.$iii['id']]))
					{
						//echo $iii['login'].' '.$key."\n";
						$m_prms['value'][]=$iii['readers'];
						$m_prms['nick'][]=(isset($arr_nau[$kkk])?($key!=''?$key:$iii['login']):($iii['login']!=''?$iii['login']:$key));//(($kkk=='vkontakte.ru') || ($kkk=='vk.com') || ($kkk=='facebook.com'))?$key:$iii['login'];
						$m_prms['srcc'][]=$kkk;
						$m_prms['id'][]=$iii['id'];
						$count_uniq_per++;
					...
					$y_prms[mb_strtolower($key,'UTF-8').':'.$iii['id']]+=$m_dinams['sp_pr'][$t][$iii['login']][$kkk]['count'];//$iii['count'];
					//print_r($y_prms);
					if ($key!=' ')
					{
		...
		foreach ($outper['value_mdint'] as $key => $item)
		{
			if ($item!=0)
			{
				...
				{
					$out['value_mdin'][$indks]['name']=$key;
					$out['value_mdin'][$indks]['count']=54014;
				}
				$indks++;
			}
		}
