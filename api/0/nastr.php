...
if ($user['tariff_id']!=3)
{
	if ($_GET['id']!='')
	{
		...
		foreach ($mid as $id)
		{
			$rs=$db->query('UPDATE blog_post SET post_nastr='.intval($_GET['value']).' WHERE post_id='.intval($id).' AND order_id='.intval($_GET['order_id']));
		}
		$mas['status']='ok';
		echo json_encode($mas);
	}
	elseif ($_GET['parent_id']!='')
	{
		...
		foreach ($mid as $id)
		{
			...
			$qparent=$db->query('SELECT parent FROM blog_post WHERE post_id='.$id.' AND order_id='.intval($_GET['order_id']).' LIMIT 1');
			$parent=$db->fetch($qparent);
			$rs=$db->query('UPDATE blog_post SET post_nastr='.intval($_GET['value']).' WHERE parent='.intval($parent['parent']).' AND order_id='.intval($_GET['order_id']));
		}
		$mas['status']='ok';
		echo json_encode($mas);
	}
