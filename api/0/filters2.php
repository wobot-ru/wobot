...
while ($row = $db->fetch($res)) 
{
	...
	//echo 'SELECT * FROM blog_tag WHERE order_id='.intval($_GET['order_id']);
	...
	if ((strtotime($_GET['start'])!=0) && (strtotime($_GET['end'])!=0))
	{
		...
		for($t=strtotime($_GET['start']);$t<=strtotime($_GET['end']);$t=mktime(0,0,0,date("n",$t),date("j",$t)+1,date("Y",$t)))
		{
			...
			foreach ($m_dinams['sp_pr'][$t] as $key => $item)
			{
				foreach ($item as $kkk => $iii)
				{
					...
					if ($key!='')
					{
						$arrleads[$iii['login']]['count']+=$iii['count'];
						$arrleads[$iii['login']]['id']=$iii['id'];
						$arrleads[$iii['login']]['hn']=$kkk;
						$arrleads[$iii['login']]['readers']=$iii['readers'];
					}
				}
			}
		}
		...
		foreach ($arrleads as $key => $item)
		{
			//echo $item['id'].' ';
			if ($kkkk>30) continue;
			if (intval($item['id'])==0) continue;
			$out['params']['promotions'][$kkkk]['nick']=$key;
			$out['params']['promotions'][$kkkk]['count']=$item['count'];		
			$out['params']['promotions'][$kkkk]['link']=$item['hn'];		
			$out['params']['promotions'][$kkkk]['id']=$item['id'];	
			$out['params']['promotions'][$kkkk]['readers']=$item['readers'];	
			$kkkk++;	
		}
		//print_r($out['params']['promotions']);
		usort($out['params']['promotions'], 'promosort');
		ksort($out['params']['source_tree']);
	}
	$i++;
}
