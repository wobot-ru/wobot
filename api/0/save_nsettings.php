...
{
	//$zap='';
	if (preg_match('/[^\d]/isu', $_POST['contact_name']) || (mb_strlen($_POST['contact_name'],'UTF-8')<10) || (mb_strlen($_POST['contact_name'],'UTF-8')>11))
	{
		$outmas['status']=7;
		echo json_encode($outmas);
		die();
	}
	$qw.=$zap.'user_contact=\''.addslashes($_POST['contact_name']).'\'';
	$zap=',';
}
if (($_POST['user_company']!='') && (mb_strlen($_POST['user_company'],'UTF-8')<=3))
{
	$outmas['status']=2;
	echo json_encode($outmas);
	die();
}
elseif ($_POST['user_company']!='')
{
	$qw.=$zap.'user_company=\''.addslashes($_POST['user_company']).'\'';
	$zap=',';
}
...
if ($_POST['user_mails']!='')
{
	$mails=explode(',',$_POST['user_mails']);
...
if (($_POST['user_position']!='') && (mb_strlen($_POST['user_position'],'UTF-8')>3))
{
...
//$us=$db->fetch($inf);
//$db->query();
if (isset($_POST['freq_mail']))
{
	$qw.=$zap.'user_freq=\''.intval($_POST['freq_mail']).'\'';
	$zap=',';
}
