...
if ((strtotime($_GET['start'])!=0) && (strtotime($_GET['end'])!=0))
{
	if ($user['tariff_id']==3)
	{
		$res=$db->query('SELECT * FROM blog_orders WHERE order_id='.intval($_GET['order_id']).' AND user_id=61');
	}
	else
	{
...
else
{
	if ($user['tariff_id']==3)
	{
		$res=$db->query('SELECT order_id,order_name,order_keyword,order_start,order_end,order_last,third_sources,order_metrics,order_src,order_graph FROM blog_orders WHERE order_id='.intval($_GET['order_id']).' AND user_id=61');
	}
	else
	{
