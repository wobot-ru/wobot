...
//if (!$loged) die();
$user['tariff_id']=1;
$user['user_id']=$_SERVER['argv'][1];
$_POST['order_id']=$_SERVER['argv'][2];
$_POST['format']='xls';
if ($user['tariff_id']==3)
{
...
while ($count=$db->fetch($countqposts))
{
	...
	if ($_POST['format']=='xls')
	{
		...
		$fp = fopen('/home/gangiman/testexport/'.$_POST['order_id'].'.json.gz', 'w');
		fwrite($fp, gzencode(json_encode($outmas), 9));
		fclose($fp);
		die();
