...
$exc_src=array('twitter.com','diary.ru','foursquare.com','ya.ru','yandex.ru','twitpic.com');
auth();
if (!$loged) die();
if ($user['tariff_id']==3)
{
	...
	if ($_POST['format']=='xls')
	{
		...
		foreach ($outcash['link'] as $key => $llink)
		{
			...
			elseif ($nastr==-1) $nstr='-';
				...
				if (!isset($mas_value[$login.':'.$hn]) && ($outcash['blog_id'][$key]!=0))
				{
					$value_count+=$comm;
		...
		if (is_resource($process))
		{
			...
			if ($user['user_email']!='')
			{
				parseUrlmail('http://wobot.ru/mail_send.php',$user['user_email'],'Команда Wobot',urlencode("<html><body>Спасибо за использование нашего сервиса, ваш отчет можно скачать по ссылке: <br><a href='http://beta.wobot.ru/digest.php?token=".$user['user_pass']."&order_id=".$idfls."&type=".$_POST['format']."&name=wobot_".date('dmy',strtotime($_POST['stime'])).'_'.date('dmy',strtotime($_POST['etime'])).'_'.$file_name."'>ссылка</a></body></html>"),$headers);
			}
			//echo ;
