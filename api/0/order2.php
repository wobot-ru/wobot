...
/*if ($user['tariff_id']!=3)
{
	$res=$db->query('SELECT order_id FROM blog_orders WHERE user_id='.intval($user['user_id']));
}
else
{
	$res=$db->query('SELECT order_id FROM blog_orders WHERE user_id=61');
	$user['user_id']=61;
}
...
else
{
	/*if ($user['tariff_id']==3)
	{
		$res=$db->query('SELECT order_id,order_name,order_keyword,order_start,order_end,order_last,third_sources,order_metrics,order_src,order_graph FROM blog_orders WHERE order_id='.intval($_GET['order_id']).' AND (user_id=61 or user_id='.intval($user['user_id']).')');
	}
	else
	{
		...
		$res=$db->query('SELECT order_id,order_name,order_keyword,order_start,order_end,order_last,third_sources,order_metrics,order_src,order_graph FROM blog_orders WHERE order_id='.intval($_GET['order_id']).' AND user_id='.intval($user['user_id']));
	}*/
	if ($user['tariff_id']==3)
	{
		$res=$db->query('SELECT * FROM blog_orders WHERE order_id='.intval($_GET['order_id']).' AND (user_id=61 or user_id='.intval($user['user_id']).')');
...
while ($row = $db->fetch($res)) 
{
	...
	foreach ($metrics['speakers']['nick'] as $key => $item)
	{
		...
		if (!isset($arr_nau[$metrics['speakers']['site'][$key]]))
		{
	...
	foreach ($metrics['promotion']['nick'] as $key => $item)
	{
		...
		if (!isset($arr_nau[$metrics['promotion']['site'][$key]]))
		{
			...
			//echo $metrics['promotion']['nick'][$key].':'.$metrics['promotion']['site'][$key]."\n";
		}
		else
		{
	...
	//echo json_encode($m_dinams['src']);
	//die();
	//echo $_GET['start'];
	//die();
	//print_r($m_dinams);
	//die();
	//echo strtotime($_GET['start']).' '.strtotime($_GET['end']);
	//die();
	if ((strtotime($_GET['start'])!=0) && (strtotime($_GET['end'])!=0))
	{
		//echo 123;
		...
		{
			$out['graph'][mktime(0,0,0,date('n',$t),date('j',$t),date('Y',$t))]=intval($timet[date('Y',$t)][date('n',$t)][date('j',$t)]);
			$count_post_per+=intval($timet[date('Y',$t)][date('n',$t)][date('j',$t)]);
			//echo intval($timet[date('Y',$t)][date('n',$t)][date('j',$t)]).' ';
			...
			//print_r($m_dinams['sp_pr'][$t]);
			foreach ($m_dinams['sp_pr'][$t] as $key => $item)
			{
				foreach ($item as $kkk => $iii)
				{
					//echo '...'.$iii['login'].' '.$key."\n";
					if ((trim($iii['nick'])=='') || (trim($iii['login'])=='')) continue;
					if (!isset($y_prms[$key]))
					{
						//echo $iii['login'].' '.$key."\n";
						$m_prms['value'][]=$iii['readers'];
						$m_prms['nick'][]=(isset($arr_nau[$kkk])?($iii['nick']!=''?$iii['nick']:$iii['login']):($iii['login']!=''?$iii['login']:$iii['nick']));//(($kkk=='vkontakte.ru') || ($kkk=='vk.com') || ($kkk=='facebook.com'))?$key:$iii['login'];
						$m_prms['srcc'][]=$kkk;
						$m_prms['id'][]=$key;
						$count_uniq_per++;
						$count_value_per+=$iii['readers'];
					}
					//echo $key.' '.$iii['login'].':'.$kkk.':'.$m_dinams['sp_pr'][$t][$iii['login']][$kkk]['count']."<br>";
					$y_prms[$key]+=$m_dinams['sp_pr'][$t][$iii['login']][$kkk]['count'];//$iii['count'];
					//print_r($y_prms);
					if ($iii['nick']!=' ')
					{
						$m_spks[$iii['nick']]+=$iii['count'];
						$m_spks1[$key]+=$iii['count'];
					}
				}
			}
			//print_r($m_dinams['uniq'][$t]);
			...
			//print_r($m_dinams['value'][$t]);
			foreach ($m_dinams['value2'][$t] as $key => $item)
			{
				foreach ($item as $kkk => $iii)
				{
					if (($key!='') && (!isset($yet_uniq_val[$key.':'.$kkk])))
					{
						$outper['value_mdint'][$kkk]+=$iii;
					}
					$yet_uniq_val[$key.':'.$kkk]++;
				}
			}
			...
			foreach ($m_dinams['words'][$t] as $key => $item)
			{
				...
				if ((trim($key)!='') && (mb_strlen($key,'UTF-8')>3))
				{
					$m_wrds[preg_replace('/[^а-яА-Яa-zA-ZёЁ]/isu','',$key)]['count']+=$item['count'];
					$m_wrds[preg_replace('/[^а-яА-Яa-zA-ZёЁ]/isu','',$key)]['word']=$item['reword'];
				}
			}
		...
		//print_r($m_prms['srcc']);
		//print_r($m_prms['nick']);
		array_multisort($m_prms['value'],SORT_DESC,$m_prms['nick'],SORT_DESC,$m_prms['srcc'],SORT_DESC,$m_prms['id'],SORT_DESC);
		arsort($m_spks);
		$kl=0;
		$indks=0;
		$k_wrds=0;
		...
		foreach ($m_wrds as $key => $item)
		{
			...
			$out['words'][$k_wrds]['word']=$item['word'];
			$out['words'][$k_wrds]['count']=$item['count'];
			$k_wrds++;
		}
		/*foreach ($m_wrds as $key => $item)
		{
		...
		foreach ($m_prms['value'] as $key => $item)
		{
			...
			$out['promotions'][$key]['count_posts']=intval($m_spks1[$m_prms['id'][$key]]);//intval($y_prms[$m_prms['nick'][$key].':'.$m_prms['srcc'][$key]]);
			...
			$out['promotions'][$key]['id']=$m_prms['id'][$key];
