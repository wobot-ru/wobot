...
if ($_POST['sum']>0)
{
	...
	//$inv_id=$db->fetch($rs);
	$rsdel=$db->query('DELETE FROM billing WHERE `status`=0 and date<'.(time()-4800));
	//$inv_id=$db->fetch($rs);
...
else
{
	...
	while ($src=$db->fetch($inf))
	{
		...
		$outmas['bill'][$kk]['money']=$src['money'];
		$outmas['bill'][$kk]['type']=($bill['money']>=0)?1:2;
		$outmas['bill'][$kk]['status']=($bill['status']==-1)?0:1;
