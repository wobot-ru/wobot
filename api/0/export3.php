...
while ($count=$db->fetch($countqposts))
{
	...
	if ($_POST['format']=='xls')
	{
		...
		while($pst4 = $db->fetch($page4_post))
		{
			...
			{
				$outmas['4'][5][]='http://plus.google.com/'.$pst4['blog_login'].'/about';
			}
			else
			{
		...
		foreach ($outcash['link'] as $key => $llink)
		{
			...
			$link=preg_replace('/\s/isu','%20',$link);
			...
			if ((trim($nick)!='') && ($hn!='.') && !preg_match('/.*\.php/isu', $login))
			{
				$uniq_mas[$nick.':'.$blog_id]++;
			...
			elseif ($nastr==-1) $nstr='-';
				...
				$outmas['header']['contacts']='e-mail:  mail@wobot.ru    phone: +7 (495) 669-27-33, +7 (968) 531-79-73';
				...
				$ss_time=explode(' ',$stime);
				if (intval($settings['export_digest'])==1)
				{
					if ($hn!='twitter.com')
					{
						$msent=get_sentence((($hn!='twitter.com')?preg_replace('/\s+/isu',' ',preg_replace('/[^а-яА-Яa-zA-ZёЁ\.\,\'\"0-9\!\?\s\/\@\:\(\)\“\_\-\&\;\#\<\>]/isu',' ',(($content_f=='')?$content:$content_f))):' '.preg_replace('/[^а-яА-Яa-zA-ZёЁ\.\,\'\"0-9\!\?\s\/\@\:\(\)\“\_\-\&\;\#\<\>]/isu',' ',(($content_f=='')?$content:$content_f))));
						// print_r($msent);
					...
					$outmas['3'][3][]=htmlspecialchars_decode(iconv("UTF-8", "UTF-8//IGNORE", $link));
					$outmas['3'][4][]=$type_re;
					$outmas['3'][5][]=iconv("UTF-8", "UTF-8//IGNORE", (($hn!='twitter.com')?preg_replace('/\s+/isu',' ',preg_replace('/[^а-яА-Яa-zA-ZёЁ\.\,\'\"0-9\!\?\s\/\@\:\(\)\“\_\-\&\;\#\<\>]/isu',' ',(($content_f=='')?$content:$content_f))):' '.preg_replace('/[^а-яА-Яa-zA-ZёЁ\.\,\'\"0-9\!\?\s\/\@\:\(\)\“\_\-\&\;\#\<\>]/isu',' ',(($content_f=='')?$content:$content_f))));
					$outmas['3'][6][]=iconv("UTF-8", "UTF-8//IGNORE", trim($digest));
					$outmas['3'][7][]=$nstr;
					$outmas['3'][8][]=$isspam;
					$outmas['3'][9][]=$isfav;
					$outmas['3'][10][]=intval($eng);
					$outmas['3'][11][]=iconv("UTF-8", "UTF-8//IGNORE", $nick);
					$outmas['3'][12][]=$gender;
					$outmas['3'][13][]=$age;
					$outmas['3'][14][]=$comm;
					$outmas['3'][15][]=$wobot['destn1'][$loc];
					$outmas['3'][16][]=$strtag;
				}
				else
				{
		...
		foreach ($top_speak['count'] as $key => $item)
		{
			...
			if ($top_speak['src'][$key]=='google.com')
			{
				$t_speak[2][]=$top_speak['link'][$key];
			}
			$t_speak[3][]=intval($top_speak['neg'][$key])+intval($top_speak['pos'][$key])+intval($top_speak['neu'][$key]);
			$t_speak[4][]=intval($top_speak['foll'][$key]);
		...
		foreach ($top_prom['count'] as $key => $item)
		{
			...
			if ($top_prom['src'][$key]=='google.com')
			{
				$t_prom[2][]=$top_prom['link'][$key];
			}
		...
		// 	}
		// }
		// $outmas['2']['audience']=intval($count_value_per);
		// $outmas['2']['uniq_auth']=count($yet_prms);
