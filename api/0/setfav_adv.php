...
if ($user['tariff_id']!=3)
{
	...
	{
		$query=query_maker($_POST,'p.post_fav='.intval($_GET['value']),'update');
		// echo $query;
		// die();
		$db->query($query);
		$mas['status']='ok';
	...
	$rs=$db->query('UPDATE blog_post SET post_fav='.intval($_GET['value']).' WHERE post_id='.intval($_GET['id']).' AND order_id='.intval($_GET['order_id']));
	$mas['status']='ok';
	echo json_encode($mas);
}
?>