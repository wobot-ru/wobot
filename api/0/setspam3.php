...
if ($user['tariff_id']!=3)
{
	...
	elseif ($_POST['type']=='parent')
	{
		...
		foreach ($ids as $id)
		{
			$qparent=$db->query('SELECT parent FROM blog_post WHERE post_id='.$id.' LIMIT 1');
			$parent=$db->fetch($qparent);
			$rs=$db->query('UPDATE blog_post SET post_spam='.intval($vl).' WHERE parent='.intval($parent['parent']).' AND order_id='.intval($_POST['order_id']));
			//echo 'UPDATE blog_post SET post_spam='.intval($vl).' WHERE post_id='.intval($id).' AND order_id='.intval($_POST['order_id']);
		}
		$mas['status']='ok';
		echo json_encode($mas);
