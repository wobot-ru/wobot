...
while ($rw = $db->fetch($rs)) 
{
	...
	if ($user['tariff_id']!=3)
	{
		...
		$res=$db->query('SELECT order_id,order_date,order_name,order_keyword,order_start,order_end,order_last,third_sources,user_id FROM blog_orders WHERE (user_id='.intval($user['user_id']).' and ut_id='.intval($rw['ut_id']).') or ((user_id=0 or user_id='.intval($user['user_id']).') and ut_id='.intval($user['ut_id']).') '.($user['user_mid']!=0?'OR (user_id='.$user['user_mid'].' AND ut_id='.$mid['ut_id'].')':''));
	}
	else
	{
		$res=$db->query('SELECT order_id,order_date,order_name,order_keyword,order_start,order_end,order_last,third_sources,user_id FROM blog_orders WHERE (user_id='.intval($user['user_id']).' and ut_id='.intval($rw['ut_id']).') or ((user_id=0 or user_id='.intval($user2).') and ut_id='.intval($ut2).') OR (user_id='.$user['user_mid'].' AND ut_id='.$mid['ut_id'].')');
	}
	while ($row = $db->fetch($res)) 
	{
		...
		$out['tariff_posts']=$rw['tariff_posts'];
		$out['user_tarif']=$rw['tariff_name'];
		$out['tarif_id']=$rw['tariff_id'];
		$out['user_money']=intval($user['user_money']);
		$out['user_email']=$user['user_email'];
		...
		$out['orders'][$i]['id']=intval($row['order_id']);
		$out['orders'][$i]['posts']=(intval($m_dinams['count_post']));
		$out['orders'][$i]['src']=(intval($m_dinams['count_src']));
		//$out['orders'][$i]['value']=formatint(intval($metrics['value']));
		$out['orders'][$i]['value']=(intval($m_dinams['value']));
		...
		//if (($row['third_sources']==2)&&(intval($m_dinams['count_post'])>0)) $out['orders'][$i]['ready']=date('d.m.Y',time());
		//if (($row['third_sources']==2)&&(intval($out['orders'][$i]['posts'])==0)) $out['orders'][$i]['ready']=false;
		if ($row['order_last']==0)
		{
			if ($row['third_sources']==0)
			{
				$out['orders'][$i]['ready']=false;
				$out['orders'][$i]['ready_perc']='0';
			}
			elseif ($row['third_sources']==1)
			{
				$out['orders'][$i]['ready']=date('d.m.Y',time());
				$out['orders'][$i]['ready_perc']='30';
			}
			elseif ($row['third_sources']==2)
			{
				$out['orders'][$i]['ready']=date('d.m.Y',time());
				$out['orders'][$i]['ready_perc']='50';
			}
			else
			{
				$out['orders'][$i]['ready']=date('d.m.Y',$row['third_sources']);
				$out['orders'][$i]['ready_perc']='100';
				// $out['orders'][$i]['ready']=date('d.m.Y',time());
				// $out['orders'][$i]['ready_perc']='70';
			}			
		}
		else
		{
			...
			{
				$out['orders'][$i]['ready']=date('d.m.Y',$row['order_last']);
				$out['orders'][$i]['ready_perc']='100';
			}
		...
		if ($row['order_date']==0 || $row['order_date']<(time()-86400))
		{
			$out['orders'][$i]['ready']=date('d.m.Y',($row['third_sources']<=2?($row['order_last']==0?mktime(0,0,0,date('n'),date('j'),date('Y')):$row['order_last']):$row['third_sources']));
			$out['orders'][$i]['ready_perc']='100';
		} 
		...
		$out['orders'][$i]['din_posts']=($m_dinams['din_posts']>999?'+999':($m_dinams['din_posts']>=0?'+'.$m_dinams['din_posts']:$m_dinams['din_posts'])).'%';
		$out['orders'][$i]['din_src']=($m_dinams['din_hn']>999?'+999':($m_dinams['din_hn']>=0?'+'.$m_dinams['din_hn']:$m_dinams['din_hn'])).'%';
		$out['orders'][$i]['div_value']=($m_dinams['din_value']>999?'+999':($m_dinams['din_value']>=0?'+'.$m_dinams['din_value']:$m_dinams['din_value'])).'%';
		$out['orders'][$i]['din_posts']='';
		$out['orders'][$i]['din_src']='';
		$out['orders'][$i]['div_value']='';
		if (intval($coll)==0)
		{
			//$out['orders'][$i]['ready']=false;
		}
	...
	//m-liner
	// if ($user['user_id']==634) $out['av_order']=33-$i;
	if ($user['user_id']==2121) $out['av_order']=10-$i;
}
...
{
	// if (intval($item['posts'])>30000) file_get_contents('http://localhost/tools/precache.php?order_id='.$item['id']);
