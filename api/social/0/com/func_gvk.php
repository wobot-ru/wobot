...
function get_group_vk($name,$start,$end,$nk,$access_token)
{
	$outmas[0]=get_main_info($name,$access_token);
	$act=get_activity($outmas[0]['gid'],$start,$end,$access_token);
	$outmas[4]=$act[1];
	...
	$k=get_k($act[0],5);
	$outmas[2]['k5']=$k;
	$outmas[1]=get_users($outmas[0]['gid'],$access_token);
	//print_r($k);
	print_r($outmas);
	return $outmas;
}
...
{
	$cont=parseUrl('https://api.vkontakte.ru/method/groups.getById?gid='.$name.'&access_token='.$access_token.'&fields=wiki_page,city,country,place,description,wiki_page,members_count,counters,start_date,end_date');
...
function get_users($groupid,$access_token)
{
	...
	$filename = "/Applications/MAMP/htdocs/stgit/stools/com/get_vk_countries.txt";
	$handle = fopen($filename, "r");
	$contents = fread($handle, filesize($filename));
	fclose($handle);
	$countries=unserialize($contents);
	$filename = "/Applications/MAMP/htdocs/stgit/stools/com/get_vk_cities.txt";
	$handle = fopen($filename, "r");
	$contents = fread($handle, filesize($filename));
	fclose($handle);
	...
	do
	{
		$id_users=parseUrl('https://api.vkontakte.ru/method/groups.getMembers?gid='.$groupid.'&count=1000&offset='.($i*1000).'&access_token='.$access_token);
		sleep(1);
		echo 'https://api.vkontakte.ru/method/groups.getMembers?gid='.$groupid.'&count=1000&offset='.($i*1000).'&access_token='.$access_token."\n";
		$ids=json_decode($id_users,true);
		$zap='';
		$qw='';
		foreach ($ids['response']['users'] as $id)
		{
			$qw.=$zap.$id;
			$zap=',';
			$arr_not_exist[$id]=1;
