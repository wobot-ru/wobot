...
if (($_GET['user_id']!='') && ($_GET['token']!='') && ($_GET['group']))
{
	...
	if (md5($user['user_email'].$user['user_pass'])==$_GET['token'])
	{
		...
		$db->query('INSERT INTO group_orders (group_link,group_start,group_end,user_id) VALUES (\''.($_GET['ag']=='acc'?'acc_':'').addslashes($out['id'][0]).'\','.$_GET['start'].','.$_GET['end'].','.$user['user_id'].')');
