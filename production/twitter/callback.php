...
$tmhOAuth = new tmhOAuth(array(
  'consumer_key'    => '3TnL1PNbUnXi7LOX2QEOjTUDO',
  'consumer_secret' => 'hcOxLUWYjvd1sq9QrAqsf2aFviIs34zUhJPRYehJtCnd8bnm0h',
));
...
// already got some credentials stored?
  ...
  if ($code == 200) {
    ...
    print_r($tmhOAuth->config);
    die();
    $key[0]['user_token']=$tmhOAuth->config['user_token'];
    $key[0]['user_secret']=$tmhOAuth->config['user_secret'];
    $key[0]['consumer_key']=$tmhOAuth->config['consumer_key'];
    $key[0]['consumer_secret']=$tmhOAuth->config['consumer_secret'];
    require_once('/var/www/userjob/com/db.php');
    require_once('/var/www/userjob/com/config.php');
    $db=new database();
    $db->connect();
    $db->query('INSERT INTO tp_keys (`key`,`type`) VALUES (\''.addslashes(json_encode($key)).'\',\'tw\')');
    echo 'INSERT INTO tp_keys (`key`,`type`) VALUES (\''.addslashes(json_encode($key)).'\',\'tw\')';
    foreach ($_GET as $key => $item)
    {
        $str.=$and.$key.'='.$item;
        $and='&';
    }
    //echo 'chrome-extension://cgameeccopapfofldjmchffjfiidnbmm/templates/default.html?'.$str;
    print_r($_SESSION);
    echo $tmhOAuth->oauth_token.' '.$tmhOAuth->config['oauth_token_secret'];
    //header('Location: chrome-extension://cgameeccopapfofldjmchffjfiidnbmm/templates/default.html?'.$str);
  } else {
