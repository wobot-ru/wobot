...
function send_mail_post($mail,$title,$content,$from)
{
	...
	curl_setopt($curl, CURLOPT_URL, 'http://188.120.239.225/api/service/sendmail.php');
...
if(isset($_POST['send']) && isset($_GET['user_id'])){
	if($_POST['text']!=""){
		...
		// parseUrlmail('http://wobot.ru/mail_send.php',$user_login,$subj,$user_text,$from);
		send_mail_post($user_login,$subj,$user_text,$from);
		//header('location:mailing.php?'.$_SERVER['QUERY_STRING']);
		header('location: http://188.120.239.225/admin/?user_id='.intval($_GET['user_id']).'#mod_user');
	}
}
...
<html>
	<head>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				var template_old='<div><?php echo $username; ?>, здравствуйте.</div>\
				...
				<div>diana@wobot-research.com</div>\
				<div>skype ask.wobot</div><div><i><img src=\'http://wobot.ru/new/assets/logo.png\'></i></div>';
				var template='<div><?php echo $username; ?>, здравствуйте.</div>\
				<div>Для доступа в систему перейдите, пожалуйста, по ссылке: <a href="production.wobot.ru">production.wobot.ru</a></div>\
				<div>Ваш логин: <?php echo $user_login; ?></div>\
				<div>Ваш пароль: <?php echo $userpass; ?></div>\
				...
				<br>\
				<div>Диана Бабенко, аккаунт менеджер</div>\
				<div>Поддержка клиентов:</div>\
				<div>+7 (495) 669 27 33,</div>\
				<div>diana@wobot-research.com</div>\
				<div>skype ask.wobot</div><div><i><img src=\'http://wobot.ru/new/assets/logo.png\'></i></div>';
	...
	<body>
		...
		<form method="post" action="?<?php echo $_SERVER['QUERY_STRING'];?>" id="form_send">
			...
			<input type="hidden" name="from" id="send_from" value="">
