...
$new_order_id=34240;
	...
	*/
	...
	$random_items=array();
	$posts=array();
	$res3=$db->query('SELECT * FROM `blog_post` WHERE order_id='.intval($_POST['order_id']).' AND post_time>='.mktime(0,0,0,8,1,2013));
	...
	while($post=$db->fetch($res3))
	{
		...
		// if (in_array($i, $random_items))
		{
			...
			$check=$db->fetch($rescheck);
			// print_r($check);
			if (intval($check['post_id'])==0)
			{
				// print_r($check);
				$qfull=$db->query('SELECT * FROM blog_full_com WHERE ful_com_post_id='.$post['post_id'].' LIMIT 1');
				$full=$db->fetch($qfull);
				// print_r($full);
				$res4=$db->query('INSERT INTO `blog_post` ('.$params.') values ('.$values.')');
				$ins_id=$db->insert_id($res4);
				// echo 'INSERT INTO blog_full_com (ful_com_post_id,ful_com_order_id,ful_com_post) VALUES (\''.$ins_id.'\','.$new_order_id.',\''.addslashes($full['ful_com_post']).'\')'."\n";
				$db->query('INSERT INTO blog_full_com (ful_com_post_id,ful_com_order_id,ful_com_post) VALUES (\''.$ins_id.'\','.$new_order_id.',\''.addslashes($full['ful_com_post']).'\')');
				// sleep(1);
			}
