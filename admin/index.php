...
function clientFilterMultiTariff($multi_array, $field, $arr_fields, $value, $logic){
	$arr=array();
	$num_fields=count($arr_fields);
	$len=count($multi_array[$field]);
	for($i=0; $i<$num_fields; $i++){
	...
	if($logic){
		for($i=0; $i<$len; $i++){
		//if($multi_array[$field][$i]==$value){
			if(in_array($multi_array[$field][$i], $value) && $multi_array[$field][$i]!=""){
				for($j=0; $j<$num_fields; $j++){
					array_push($arr[$arr_fields[$j]], $multi_array[$arr_fields[$j]][$i]);
				}
			}
...
elseif ($_POST['action']=='edituser')
{
	if ((intval($_POST['user_id'])!=0)&&($_POST['user_email']!='')&&($_POST['user_name']!='')&&($_POST['user_contact']!='')&&($_POST['user_company']!=''))
	{
		if ($_GET['topipe']==1)
		{
			...
			$db->query('UPDATE users set ref=1 WHERE user_id='.intval($_POST['user_id']));
			...
			/*
			19eb507bf7bb30b6075acce3398d2758
			USER_LOGIN=Логин&USER_HASH=ключ
			/private/api/contact_add.php
			ACTION=ADD_PERSON
contact = Array(
...
//print_r($_POST);
	if ((intval($_POST['order_id'])!=0)&&($_POST['order_name']!='')&&($_POST['order_keyword']!='')&&($_POST['order_start']!='')&&($_POST['order_end']!='')&&(intval($_POST['ut_id'])!=0))
	{
		...
		{
			$cont=parseUrl('http://localhost/tools/cashjob.php?order_id='.intval($_POST['order_id']));
		}
		//echo 'Тариф обновлен<br>';
	}
	else
...
elseif ($_POST['action']=='recoverorder')
{
	...
	if ((intval($_POST['user_id'])>0) && (intval($_POST['order_id'])>0))
	{
		...
		parseUrl('http://localhost/tools/charge.php?order_id='.intval($_POST['order_id']));
...
{
	$cont=parseUrl('http://localhost/tools/start_dup.php?user_id='.intval($_GET['user_id']));
}
elseif (($_POST['action']=='deleteuser') || (intval($_GET['delete_user_id'])!=0))
{
	...
	if ($_POST['action']=='deleteuser')
	{
		...
		$db->query('UPDATE users set user_email=concat(user_email, \'_deleted\') WHERE user_id='.intval($_POST['user_id']));
	}
	elseif (intval($_GET['delete_user_id'])!=0)
	{
		...
		$db->query('UPDATE users set user_email=concat(user_email, \'_deleted\') WHERE user_id='.intval($_GET['delete_user_id']));
...
elseif ($_POST['action']=='addnewsrc')
{
	...
	foreach ($msrc as $item)
	{
		...
		if (($hn!='') && ($hn!='.'))
		{
			...
			if ($is_yet==0)
			{
				parseUrl('http://localhost/tools/simpl_addsrc.php?src='.$db->insert_id());
            ...
            //установка ширины и высоты textarea в таблице тарифов
                ...
                $("#sort_date, #sort_alf, #sort_ut, #sort_tariff").click(function(){
                    if($("i", this).attr("class")=="icon-arrow-up"){
						...
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=ctime&type=asc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
						else if ($(this).attr("id")=="sort_ut")
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=ut_date&type=asc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
						else if ($(this).attr("id")=="sort_tariff")
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=tariff&type=asc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
						else
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=alph&type=asc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
						...
						if ($(this).attr("id")=="sort_date")
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=ctime&type=desc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
						else if ($(this).attr("id")=="sort_ut")
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=ut_date&type=desc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
						else if ($(this).attr("id")=="sort_tariff")
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=tariff&type=desc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
						else
						{
							window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&sort=alph&type=desc&tfilter='.$_GET['tfilter'].'&payed='.$_GET['payed'].'&tmultifilter='.$_GET['tmultifilter'].'&tmultiarray='.$_GET['tmultiarray'].'&tmultitype='.$_GET['tmultitype'].'#client_list";
						}
				...
				$(".tariff_control").hover(function(){
					$(".tariffs_dropdown").show();
				}, function(){
					$(".tariffs_dropdown").hide();
				});
				
				$("#tariff_only").click(function(){
					var res = [];
					var payed = 0;
					$(".tariffs_dropdown .tariff_select").each(function(){
						if($(this).is(":checked")){
							res.push($(this).val())
						}
					});
					if($("#tariff_multi_payed").is(":checked")){
						payed = 1;
					}
					console.log(res.join());
					window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&tmultifilter=1&tmultiarray="+res.join()+"&tmultitype=only&payed="+payed+"#client_list";
				});
				
				$("#tariff_except").click(function(){
					var res = [];
					var payed = 0;
					$(".tariffs_dropdown .tariff_select").each(function(){
						if($(this).is(":checked")){
					...
					console.log(res.join());
					window.location = "http://188.120.239.225/admin/?user_id='.$_GET['user_id'].'&tmultifilter=1&tmultiarray="+res.join()+"&tmultitype=except&payed="+payed+"#client_list";
				});
				...
				foreach ($stus as $key => $item)
				{
					echo '
	                    <tr>
		                    ...
		                    <td><a href="?toPD='.$key.'"><i class="icon-briefcase"'.((intval($item['ref'])==1)?' style="background-color: #00ff00;"':'').' title="Экспорт в AmoCRM"></i></a></td>
							<td><a href="?user_id='.$key.'#mod_user"><i class="icon-pencil" title="редактировать"></i></a></td>
		                    <td><a href="?delete_user_id='.$item['user_id'].'#mod_user" onclick="return confirm(\'Удалить пользователя?\');"><i class="icon-remove" title="Удалить"></i></a></td>
                        ...
                        <div class="tab-content"> <!--содержимое вкладок алфавит-->
                            <div class="tab-pane active" id="all">
                                <div class="alfabet_cont">
                                    ...
                                    echo '<button class="btn" id="apply_tfilter">Применить</button></div>
          							<div class="tariff_control">
	          								
	          							<div class="tariff_control_head"><a href="#">Фильтр тарифов</a></div>

	                                    <div class="tariffs_dropdown">';
	                                    
	                                    	foreach ($tariffs as $key => $item)
											{
												if($item['tariff_name']!=""){
													echo '<div><input class="tariff_select" style="margin: 0" type="checkbox" value="'.$item['tariff_id'].'"><span> '.$item['tariff_name'].'</span></div>';
												}
											}

		                                    echo '
		                                    <div>
		                                    	<button class="btn" id="tariff_only">Только</button> <button class="btn" id="tariff_except">Кроме</button>
		                                    	<input type="checkbox" id="tariff_multi_payed" value="" placeholder=""> Оплачено
		                                    </div>
	                                    </div>
	                                </div> <!-- tariff control end -->
									...
									if(isset($_GET['tmultifilter']) && $_GET['tmultifilter']=='1' && isset($_GET['tmultiarray']) && $_GET['tmultiarray']!='' && isset($_GET['tmultitype']) && $_GET['tmultitype']!=''){
										if($_GET['tmultitype']=="only"){
											$logic = true;
										} else {
											$logic = false;
										}
										$allus=clientFilterMultiTariff($allus,"tariff_id",$farr,explode(",",$_GET['tmultiarray']), $logic);
									}
							...
							foreach ($mus as $key => $item)
							{
								...
								foreach ($mus[$key]['email'] as $k => $i)
								{
									echo '
									<tr>
                                        <td >'.$mus[$key]['email'][$k].'</td><td style="width: 50px;"><a href="?user_id='.$mus[$key]['id'][$k].'#mod_user"><i class="icon-pencil" title="редактировать"></a></td><td style="width: 50px;"><a href="?delete_user_id='.$mus[$key]['id'][$k].'#del_'.$key.'" onclick="return confirm(\'Удалить пользователя?\');"><i class="icon-remove" title="Удалить"></i></a></td>
                                    </tr>';
								}
								echo '</table>
								</div>
                    ...
                    <div class="inner-tab-container"> <!--mod_user tab container-->
                                    ...
                                    <form method="post" action="?user_id='.$user['user_id'].'#mod_user">
	                                    ...
	                                    if($us_tariff['tariff_id']==16){
	                                    	echo '<a style="margin-top:-9px;" class="btn" href="/admin/mailing.php?user_id='.$user['user_id'].'&firsttheme=1" target="_blank">Отправить письмо </a>';
	                                    }
                                    echo '</form>
									...
									<input class="btn" type="button" value="==>AmoCRM" onclick="document.getElementById(\'main_formm\').action=\'?user_id='.$user['user_id'].'&topipe=1#mod_user\'; document.getElementById(\'main_formm\').submit();">
                            	 ...
                            	 <div style="height: 15px;"></div>
                                    <a style="margin-top: 10px;" href="/tools/groups_tp.php?user_id='.$_GET['user_id'].'">[ Добавить группы из социальных сетей пользователю ]</a><br>
                                    <a style="margin-top: 10px;" href="/tools/addnews.php?user_id='.$_GET['user_id'].'">[ Добавить выдачу с news.yandex.ru пользователю ]</a><br>
                                    <div style="height: 15px;"></div>
                                    ...
                                    <a href="/tools/tp_info.php" target="_blank">[ Статистика реал-тайм сбора ]</a>
                                    <div style="height: 15px;"></div>
                                    	<b>История счетов</b>
                                    <div style="height: 15px;"></div>
