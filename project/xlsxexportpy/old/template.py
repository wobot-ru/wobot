...
def log_input(f):
    def af(*args,**kwargs):
        with open("{0}.log".format(f.__name__),"w+") as fd:
            fd.write(json.dumps({'args':args, 'kwargs': kwargs}, indent=4))
...
class Template:
    review_comments = [u"На которых мы нашли упоминания по вашей теме",
                       ...
                       u"К упоминаниям из выдачи. Учитываются комментарии с популярных ресурсов, например, livejournal.ru, vk.com",
    ...
    # time_distribution_column = [u"0 - 1", u"1 - 2", u"2 - 3", u"3 - 4", u"4 - 5", u"5 - 6", u"6 - 7", u"7 - 8", u"8 - 9", u"9 - 10",
    #                             u"10 - 11", u"11 - 12", u"12 - 13", u"13 - 14", u"14 - 15", u"15 - 16", u"16 - 17", u"17 - 18", u"18 - 19",
    #                             u"19 - 20", u"20 - 21", u"21 - 22", u"22 - 23", u"23 - 24"]
    ...
    popular_mentions_small_table = [u"Доминирующий ресурс", u"Охват по доминирующему ресурсу", u"Индекс тональности популярных постов"]
    popular_mentions_comment = [u"Среди 10 самых популярных",u"От охвата по всем упоминаниям темы",
                                u"Отношение количества положительно окрашенных популярных постов к числу отрицательно окрашенным."]
    resource_small_table = [u"Ресурс", u"Упоминаний", u"Охват"]
    authors_table_header = [u"№", u"Автор", u"Ссылка на профиль", u"Читателей", u"Упоминаний", u"Комментариев", u"Лайков", u"Ретвитов",
                            u"Положительных", u"Негативные", u"Нейтральных", u"Ресурс", u"Пол", u"Возраст", u"Регион"]
    tonal_table_header = [u"Дата", u"Время", u"Полный текст", u"Ссылка", u"Охват", u"Вовлеченность", u"Лайки", u"Комментарии",
    ...
    characteristics_date_per_age = [u"Дата", u"до 14", u"15-18", u"19-25", u"26-30", u"31-35", u"36-45", u"46-55", u"от 55", u"Неопределено"]
    tonal_all_table_column = [u"Нейтральные", u"Негативные", u"Положительные"]
    characteristics_gen_table_column = [u"М", u"Ж", u"Неопределено"]
    characteristics_age_table_column = [u"до 14", u"15-18", u"19-25", u"26-30", u"31-35", u"36-45", u"46-55", u"от 55", u"Неопределено"]
    ...
    def __init__(self, filename,data_json):
        ...
        self.right_bottom_gray_format = self.workbook.add_format({'right': 4, 'bottom': 4, 'bg_color': '#F2F2F2', 'text_wrap': True})
        self.left_top_white_format = self.workbook.add_format({'left': 4, 'top': 4, 'text_wrap': True})
        self.left_top_gray_format = self.workbook.add_format({'left': 4, 'top': 4, 'bg_color': '#F2F2F2', 'text_wrap': True})
        ...
        attributes = {'bold': True, 'align': 'center', 'valign': 'vcenter', 'font_color': '#404040','bottom': 1, 'right': 4}
        ...
        self.link_format = self.workbook.add_format({'font_color': '#009900','underline': True,'font_size': 12, 'bold': True, 'valign':'bottom'})
        self.linkRow = 6
        self.header_data = [data_json["Sh1"]["order_name"], data_json["Sh1"]["order_time"], data_json["Sh1"]["order_keyword"]]
    ...
    def addTonalTable(self, worksheet, data):
        top_header = self.workbook.add_format({'bold': True, 'align': 'center', 'valign': 'vcenter','text_wrap': True, 'font_color': '#404040'})
        for i, value in enumerate([u"Количество положительных",u"Количество негативных"]):
            worksheet.write(7, 3 + i,value,top_header)
        left_odd_header = self.workbook.add_format({'right':4, 'bottom':4, 'top':4, 'valign': 'vcenter', 'align': 'right','bg_color':'#F2F2F2'})
        left_even_header = self.workbook.add_format({'right':4, 'bottom':4, 'top':4, 'valign': 'vcenter', 'align': 'right', 'bold': True})
        ...
        dark_green_format = self.workbook.add_format({'bottom':4, 'top':4, 'valign': 'vcenter', 'align': 'right','right':4,
                                                      'bold': True, 'bg_color':'#C4D79B','font_color': '#4F6228','font_size': 16})
        light_green_format = self.workbook.add_format({'bottom':4, 'top':4, 'valign': 'vcenter', 'align': 'right','right':4,
                                                      'bold': True, 'bg_color': '#EBF1DE','font_color': '#4F6228','font_size': 16})
        dark_orange_format = self.workbook.add_format({'bottom':4, 'top':4, 'valign': 'vcenter', 'align': 'right',
                                                      'bold': True, 'bg_color':'#FABF8F','font_color': '#974706','font_size': 16})
        light_orange_format = self.workbook.add_format({'bottom':4, 'top':4, 'valign': 'vcenter', 'align': 'right',
                                                      'bold': True, 'bg_color':'#FDE9D9','font_color': '#974706','font_size': 16})
    ...
    def addInternalLink(self,worksheet_name):
        self.review.write_url(self.linkRow,5,u"internal:'%s'!A1" % worksheet_name)
        self.review.write(self.linkRow,5,worksheet_name,self.link_format)
        self.linkRow += 1
    ...
    def insertChartV1(self,worksheet, datarange, categoryrange, chart_axis_title,target_cell):
        ...
        worksheet.insert_chart(target_cell,chart) #,{'x_offset': 0, 'y_offset': 10})

    def fillReviewWorksheet(self,data):
        summary_column = [data['count_posts'], data['uniq_auth'], data['post_in_day'], data['count_hosts'], data['audience'],
                          data['likes_count'], data['comments_count'], data['retweet_count'], data['city_count'] + data['country_count']]
        self.addTableWithFormatting(self.review, 6, 1, [self.summary_table, summary_column], start_gray_row = 2,
                                    column_formatting=[[('text_wrap', True)], [('bold', True),('align','right'),('font_size', 16),('indent',1)]])
        self.addTableWithFormatting(self.review, 60, 1, data['graph'], start_gray_row = 1,column_formatting = [[('align', 'left'),('indent', 1)]] * 2)
        ...
        datarange = ("C", "61", "C", str(60 + len(data['graph'][0])))
        categoryrange = ("B", "61", "B", str(60 + len(data['graph'][0])))
        self.insertChartV1(self.review,datarange ,categoryrange ,u"Количество упоминаний",'B19')
        datarange = ("F","61","F","84")
        categoryrange = ("E","61","E","84")
        self.insertChartV1(self.review,datarange ,categoryrange ,u"Упоминаний",'B40')
    ...
    def makeMentionsWorksheet(self):
        ...
        rows = {4: 23.5, 5: 15}
        columns = {0: 2.18, 1: 11.64, 2: 11.45, 3: 19.09, 4: 53.09, 5: 53.09, 6: 30, 7: 6.18, 8: 6.18, 9: 6.18, 10: 8.45,
                   11: 8.45, 12: 8.45, 13: 32.18, 14: 8.45, 15: 8.45, 16: 20.36, 17: 17.73, 18: 8.45, 19: 8.45}
    ...
    def fillMentionsWorksheet(self,data, tag_count):
        ...
        mentions_column_formatting = [[('align', 'center')]] * 2 + [[]] * 4 + [[('align', 'center')]] * 3 + [[]] * 4 + [[('align', 'center')]] * 4 + [[]] * 2 + [[]] * tag_count
    ...
    def makePopularMentionsWorksheet(self):
        ...
        self.popular_mentions.merge_range(5, 1, 5, 5,u'10 упоминаний с наибольшим значением вовлеученности. Вовлеченность — активность и заинтересованность аудиториии, прочитавшей пост. Рассчитывается как сумма комментариев, перепостов и отметок "Мне нравится"', self.m_subtitle_format)

        self.popular_mentions.merge_range(7,1,7,3,self.popular_mentions_small_table[0],self.right_bottom_gray_format)
        self.popular_mentions.merge_range(8,1,8,3,self.popular_mentions_small_table[1],self.right_bottom_white_format)
        self.popular_mentions.merge_range(9,1,9,3,self.popular_mentions_small_table[2],self.gray_format)

        self.popular_mentions.write_column(7,5,self.popular_mentions_comment,self.comment_format)
    ...
    def fillPopularMentionsWorksheet(self,data):
        ...
        self.addTableWithFormatting(self.popular_mentions, 12, 1, data['top'],column_formatting = popular_mentions_column_formatting)
        ...
        self.popular_mentions.write(7, 4, data["dominant_host"], self.gen_format({'bg_color': '#F2F2F2'}, table_format_attributes))
        self.popular_mentions.write(8, 4, data["dominant_host_engage"], self.gen_format({'left': 4, 'top': 4}, table_format_attributes))
        self.popular_mentions.write(9, 4, data["tonalIndex"],self.gen_format({'left': 4, 'top': 4, 'bg_color': '#F2F2F2'}, table_format_attributes))
    ...
    def makeResourceWorksheet(self):
        ...
        self.resources.merge_range(5,1,6,7,u"5 ресурсов с наибольшим количеством упоминаний. Охват — объем потенциальной аудитории сообщения. Рассчитывается как сумма количества друзей, подписчиков или последователей автора упоминания.",self.m_subtitle_format)
    ...
    def fillResourceWorksheet(self, data):
        ...
        top_count_column_format = self.generate_alignment("LCRCR", percentage_position=2)
        ...
        categoryrange = ("F","43","F",last_row)

        for idx,column in enumerate("GHIJK"):
            chart.add_series({
                'values': ("%s!$%s$%s:$%s$%s" % ((self.resources.name,column,"43",column,last_row))),
                'categories': ("%s!$%s$%s:$%s$%s" % ((self.resources.name,) + categoryrange)),
                'name' : top_count_header[idx+1],
        ...
        self.incertPieChart(self.resources,'H9',[9, 3, 14, 3],[9, 1, 14, 1],{'x_offset': 25, 'y_offset': 0})

    def incertPieChart(self, worksheet, cell_to_incert, data_range, category_range, offsets, chart_size=None):
    ...
    def makeAuthorsWorksheet(self):
        ...
        columns = {0:2.18, 1:4.71, 2:21.14, 3:30, 4:7.29, 5:8.71, 6:6, 7:6, 8:6, 9:6, 10:6, 11:7.29, 12:4.71, 13:5.71, 14:15.71, 15:19.71}
    ...
    def makeLeadersWorksheet(self):
        ...
        rows = {3: 18.75, 4: 26.25, 5: 30, 6: 13.5, 7: 18.75, 8: 18.75, 9: 18.75, 10: 18.75, 11: 18.75, 12: 18.75, 13: 18.75, 14: 18.75, 15: 18.75, 16: 18.75, 17: 18.75, 18: 18.75, 19: 25.5, 20: 18.75, 21: 18.75,47:30}
        columns = {0: 2.18, 1: 5, 2: 22.71, 3: 30, 4: 10.71, 5: 6, 6: 6, 7: 6, 8: 6, 9: 6.57, 10: 6.57, 11: 6.57, 12: 4.71, 13: 5.71, 14: 15.71, 15: 20}
        ...
        self.leaders.merge_range(5, 1, 5, 5, u"Сообщества и персоны с наибольшей аудиторией.  Охват рассчитывается как сумма количества друзей, подписчиков или последователей автора упоминания.", self.subtitle_format)
        for row in [20, 65]:
        ...
        self.leaders.merge_range(47, 1, 47, 5, u"Сообщества и персоны с наибольшим количеством упоминаний.", self.subtitle_format)

    def insertColumnChart(self, worksheet, cell_to_incert, datarange, categoryrange):
        column_chart = self.workbook.add_chart({'type':'column'})
        column_chart.add_series({
            'data_labels': {'value': True,
                            'position': 'inside_end'},
            'categories': [worksheet.name,] + categoryrange,
            'values':     [worksheet.name,] + datarange,
            'fill':   {'color': '#92D050'},
        })
    ...
    def fillLeadersWorksheet(self, data):
        ...
        self.addTableWithFormatting(self.leaders,23,1,data["audience"],start_gray_row=1,column_formatting=table_alignment)
        self.addTableWithFormatting(self.leaders,68,1,data["count"],start_gray_row=1,column_formatting=table_alignment)
        table_height = 10
        if len(data["count"][0])<10:
            table_height = len(data["count"][0])
        chart_params = [("B8",[23, 4, 22 + table_height, 4],[23, 2, 22 + table_height, 2]),("B49",[68,5,67 + table_height,5],[68,2,67 + table_height,2])]
    ...
    def makeTonalWorksheet(self):
        ...
        rows = {4: 23.25, 5: 18, 7: 33, 8: 21, 9: 21, 10: 16.5, 11: 20.25, 12: 15.75, 13: 22.5, 14: 22.5, 15: 32.25, 16: 23.25}
        columns = {0: 2.18, 1: 9.14, 2: 13, 3: 28, 4: 28, 5: 7.57, 6: 15.57, 7: 6.57, 8: 5.71, 9: 5.57, 10: 20, 11: 5.57, 12: 6.43, 13: 17.43, 14: 18.71, 15: 18.43, 16: 8.14, 17: 16.14, 18: 8.14, 21: 8.43, 22: 8.43, 23: 8.43}
        self.adjustRowsAndColumns(self.tonal, rows, columns)
        titles = [(4,u"Тональность упоминаний"),(16,u"Динамика тональности упоминаний"),(34,u"Положительные посты с наибольшим охватом"),
            (49,u"Негативные посты с наибольшим охватом"),(63,u"Тональность упоминаний по дням")]
    ...
    def fillTonalWorksheet(self, data):
        ...
        self.addTableWithFormatting(self.tonal, 37,1,data["top_positive"],start_gray_row=1,column_formatting=top_posneg_table_formatting)
        self.addTableWithFormatting(self.tonal, 52,1,data["top_negative"],start_gray_row=1,column_formatting=top_posneg_table_formatting)
        self.addTableWithFormatting(self.tonal, 66,1,data["dinams"],start_gray_row=1)
        self.addTableWithFormatting(self.tonal, 66,6,[self.tonal_all_table_column] + data["all"],start_gray_row=1,column_formatting=tonal_all_table_column_format )
        self.incertPieChart(self.tonal,'G7',[66, 7, 68, 7],[66, 6, 68, 6],{'x_offset': 25, 'y_offset': 0})
    ...
    def fillCharacteristicsWorksheet(self,data):
        ...
        self.incertPieChart(self.characteristics, "J7", [26, 10, 28, 10],[26, 8, 28, 8],{'x_offset': 55, 'y_offset': 11})
    ...
    def fillTagWorksheet(self,data):
        ...
        self.incertPieChart(self.tags, "I7", [8, 2, last_row, 2],[8, 1, last_row, 1], {'x_offset': 20, 'y_offset': 11}, chart_size={'width': 520, 'height': 460})
