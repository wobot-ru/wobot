...
while($blog=$db->fetch($ressec))
{
	...
	$link='https://graph.facebook.com/search?q='.urlencode($query).'&type=post&limit=75&access_token=&locale=ru_RU'.(intval($blog['order_fb_rt'])>0?'&since='.intval($blog['order_fb_rt']):'');
	echo $link;
	$out=parseurl2($link);
	//echo "==============================================\n";
	//echo $out;
