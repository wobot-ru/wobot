...
//http://bmstu.wobot.ru/project/facebook-manual.php?id=&token=
//$_GET['id']='557';
//$_GET['token']='AAACQNvcNtEgBADuAectSakmuPVf5PLaXYnRmJu0lEVi30TvXDzPHOIgTTZAYlZCbbaqhhMkNkrzFeJ9V4oBugbF0m1VoZA2cRds5l0Xkai9ZAZCrFZAq0o';
...
$ressec=$db->query('SELECT * FROM blog_orders WHERE order_id='.intval($_GET['id']));
//order_fb_rt!=0 and order_fb_rt<(order_end+86400) and
echo 'new orders to cache: '.mysql_num_rows($ressec)."\n";
while($blog=$db->fetch($ressec))
{
	$i=0;
	$mqu=explode('|',$blog['order_keyword']);
	foreach ($mqu as $mqit)
	{
		$mqit=str_replace('(',' ',$mqit);
		$mqit=str_replace(')',' ',$mqit);
		$mqit=str_replace('&&',' ',$mqit);
		$mqit=str_replace('&',' ',$mqit);
		$mqit=str_replace('"',' ',$mqit);
		list($mqit,$tmp)=explode('~',$mqit,2);
		$mqit=preg_replace('/\s+/u',' ',$mqit);
	...
	//simple
	if ($blog['order_fb_rt']<$blog['order_start']) $blog['order_fb_rt']=$blog['order_start'];
	$blog['order_fb_rt']=0;
	$link='https://graph.facebook.com/search?q='.urlencode($query).'&type=post&limit=75&access_token='.$_GET['token'].'&locale=ru_RU'.(intval($blog['order_fb_rt'])>0?'&since='.intval($blog['order_fb_rt']):'');
	//echo $link."\n\n";
	$out=parseurl2($link);
	//echo "==============================================\n";
	//echo $out;
	...
	[data] => Array
		...
		foreach ($data['data'] as $post)
		{
			...
			{
				//$db->query('INSERT INTO blog_post (order_id, post_link, post_time, post_content, blog_id, post_engage) values ('.$blog['order_id'].',"http://www.facebook.com/'.$part1.'/posts/'.$part2.'",'.$post_time.',"'.$post_content.'",'.intval($blog_id).','.intval($post['likes']['count']).')');
				echo $blog['order_id'].' http://www.facebook.com/'.$part1.'/posts/'.$part2.' ['.$post_time.'] '.$post_content.'",'.intval($blog_id).','.intval($post['likes']['count'])."<br>";
				$i++;
			}
		}
