...
function set_log($type,$data)
{
    global $redis;
    $var=$redis->get('log_activity'.mktime(date('H'),0,0,date('n'),date('j'),date('Y')));
    $types=json_decode($var,true);
    $types[$type]['count']++;
    $types[$type]['times'][mktime(0,0,0,date('n'),date('j'),date('Y'))]['count']++;
    $types[$type]['hashs'][md5(json_encode($data))]['count']++;
    $types[$type]['hashs'][md5(json_encode($data))]['query']=json_encode($data);
    // $redis->rPush('log_queue', json_encode(array('data'=>$data,'type'=>$type)));
    //print_r($types);
    //unset($types);
    //unset($types);
    //echo $count;
    $redis->set('log_activity'.mktime(date('H'),0,0,date('n'),date('j'),date('Y')), json_encode($types));
}
