...
if ($_GET['submitted']==true)
{
	...
	{
		if ((substr($inddd, 0, 8)=='user_tag'))
		{
			$uptag[]=$posttt;
			$uptag1[]=substr($inddd, 8, 8);
		}
	}
	//print_r($dtags);
	foreach ($uptag as $kkk => $ttg)
	{
		$res=$db->query('UPDATE blog_tag SET tag_name=\''.$ttg.'\' WHERE (user_id='.$_GET['orid'].' AND tag_tag='.$uptag1[$kkk].')');
		//echo 'UPDATE blog_tag SET tag_name=\''.$ttg.'\' WHERE (user_id='.$_GET['orid'].' AND tag_tag='.$uptag1[$kkk].')';
	}
}
	...
	<input type="hidden" name="oorid" value="'.intval($_GET['oorid']).'" />
		<div class="forms">
			...
			while ($row = $db->fetch($res)) 
			{
				...
				<div class="pright"><input type="text" class="text" onkeydown="if ($(\'#user_name'.$id.'\').attr(\'value\').length>10) {$(\'#user_name'.$id.'\').attr(\'value\',$(\'#user_name'.$id.'\').attr(\'value\').substr(0,$(\'#user_name'.$id.'\').attr(\'value\').length-1));}" name="user_tag'.$row['tag_tag'].'" id="user_name'.$id.'" value="'.$row['tag_name'].'" /></div>';
			}
