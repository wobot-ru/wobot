...
$ressec=$db->query('SELECT * FROM blog_order where order_id='.intval($_GET['order']));
...
while($blog=$db->fetch($ressec))
{
	...
	foreach ($posts as $gg => $post)
	{
			...
			if ($hn=='twitter.com') {
				...
				{
					//$man=$db->fetch($bman);
					$allposts[]=array('content'=>$content,'nick'=>$login/*$nick*/,'readers'=>$post['reads'],'site'=>'twitter');
			...
			if (($hn=='livejournal.com') || ($hn=='vkontakte.ru') || ($hn=='facebook.com'))
			{
				...
				{
					//$man=$db->fetch($bman);
					$allposts[]=array('content'=>$content,'nick'=>$login/*$nick*/,'readers'=>$post['reads'],'site'=>$hh);
					//echo $hn." ".$nick." ".$man['blog_readers']."\n";
					//array_push($allposts,array('content'=>$content,'nick'=>$man['blog_login'],'readers'=>$man['blog_readers'],'site'=>'twitter'));
				}
			}
	...
	foreach ($metrics['speakers']['nick'] as $pnick => $mas)
	{
		foreach ($posts as $ppp)
		{
			if ($ppp['nick']==$mas)
			{
				...
				$metrics['speakers']['link'][$pnick]=$ppp['blog_link'];
	...
	foreach ($metrics['promotion']['nick'] as $pnick => $mas)
	{
		foreach ($posts as $ppp)
		{
			...
			{
				//echo $ppp['nick']." ".$ppp['blog_link'];
				$metrics['promotion']['link'][$pnick]=$ppp['blog_link'];
