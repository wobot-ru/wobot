...
$server=$config_server['server_host'];
...
$assoc_name_daemon['egtw']='adv_engage';
$assoc_name_daemon['alert']='alertjob';
$assoc_name_daemon['cu']='cash_updater';
$assoc_name_daemon['reaction']='reaction';
...
sleep(5);
if (!empty($fails)) {
$output.='<tr><td colspan="3" align="center"><i>после перезапуска</i></td></tr>';
foreach ($fails as $name)
{
	...
	//if ($daemon_work==$count) 
	{
		$output.='<tr><td>'.$name.'</td><td>'.$daemon_work.'/'.$count_config[$name].'</td><td>'.($daemon_work==$count_config[$name]?'<p style="color: green;">OK</p>':'<p style="color: red;">FAIL</p>').'</td></tr>';
	}
	// print_r($mcont_shell);
}
}
...
*/
if (!empty($fails)) {
// parseUrlmail('http://188.120.239.225/api/service/sendmail.php','wobottest@yandex.ru','Краткая статистика работы сервера ('.$server.') '.$addition_title,$output,'noreply2@wobot.ru');
}
?>