...
while($blog=$db->fetch($ressec))
{
	...
	$query='SELECT * from blog_post as p LEFT JOIN robot_blogs2 AS b ON p.blog_id=b.blog_id WHERE p.order_id='.$blog['order_id'].' AND post_time>'.(intval($blog['order_start'])-86400).' AND post_time<='.intval($enddd).' order by post_time ASC';
	echo $query;
	...
	foreach ($posts as $gg => $post)
	{
			...
			if ($hn=='twitter.com') {
				...
				{
					if (!isset($yet_users[$login.':'.$hn]))
					{
						$vvv+=$post['reads'];
						$value_din[$hn]+=$post['reads'];
						$beta_data['value'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$hn]+=$post['reads'];
						$beta_data['uniq'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$login1][$hn]=$post['reads'];
					}
					$yet_users[$login.':'.$hn]++;
			...
			if (($hn=='livejournal.com') || ($hn=='vkontakte.ru') || ($hn=='facebook.com') || ($hn=='vk.com'))
			{
				...
				//if (mysql_num_rows($bman)>0)
				{
					...
					if ((strpos($link, 'thread')!==false) && ($hn='livejournal.com'))
					{
						...
						$login1=$out['nk'][0];
						if (!isset($prom_info[$login1]))
						{
							$bl_info=$db->query('SELECT * FROM robot_blogs2 WHERE blog_login=\''.$login1.'\' AND blog_link=\'livejournal\.com\' LIMIT 1');
							$innf=$db->fetch($bl_info);
							$readers_prom=$innf['blog_readers'];
							$prom_info[$login1]=$readers_prom;
							$blog_id=$innf['blog_id'];
						}
						else
						{
							$readers_prom=$prom_info[$login1];
						}
						//echo $login1.' '.$readers_prom.' '.."\n";
					}
					if (!isset($yet_users[$nick.':'.$hn]))
					{
						$vvv+=$readers_prom;
						$value_din[$hn]+=$readers_prom;
						$beta_data['value'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$hn]+=$readers_prom;
						$beta_data['uniq'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$nick][$hn]=$readers_prom;
					}
					$yet_users[$nick.':'.$hn]++;
					//$yet_users1[$nick.':'.$hn]++;
