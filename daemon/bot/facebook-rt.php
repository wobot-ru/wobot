...
while(1)
{
$ressec=$db->query('SELECT * FROM blog_orders where order_id=447 OR order_id=448 OR order_id=449 OR order_id=450');//order_id=441 OR
echo 'new orders to cache: '.mysql_num_rows($ressec)."\n";
while($blog=$db->fetch($ressec))
{
	$keyword=(strlen($blog['order_keyword'])>0?$blog['order_keyword']:$blog['order_name']);
	$keyword=str_replace('\"','',$keyword);
	$qmas=explode('|',$keyword);
	foreach ($qmas as $iqmas)
	{
	$query=$iqmas;//$blog['order_name'];
	echo '['.date('r').'] '.$query;
