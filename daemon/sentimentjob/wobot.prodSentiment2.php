...
class prodSentiment extends complexSentiment
{
    ...
    function __construct($order_id, $mstart = '', $mend = '')
    {
        ...
        {
            echo "NO DATES\n";
            if (!$this->getLastCheckedDate()) return false;
        }
        $this->getCorpora();
    ...
    private function getLastCheckedDate()
    {
        ...
        if (empty($order)) return false;//die('DIE!EDDE!!!111'.json_encode($order).' '.$this->order_id);
...
function handleShutdown() {
    ...
    if($error !== NULL){
        ...
        mailerror("r@wobot.co", $info);
        //mailerror("nikanorov@wobot.co, for.uki@gmail.com", $info);
        //yourPrintOrMailFunction($info);
    }
    else{
        mailerror("zmei123@yandex.ru,r@wobot.co", "Упал sentimentjob shutdown");
...
while (1)
{
    ...
    $ressec = $db->query('SELECT * FROM blog_orders WHERE order_nastr!=0'.$condition.'
                        AND MOD (order_id, '.$_SERVER['argv'][2].') = '.$_SERVER['argv'][1].' ORDER BY order_id DESC');
    ...
    while ($blog = $db->fetch($ressec))
    {
        ...
        if ($blog['order_id']==2867) continue;
        ...
        if (!$sent = new prodSentiment($blog['order_id'])) continue;
        $sent->prodController();
