...
while (1)
{
	$qproxy=$db->query('SELECT * FROM tp_proxys WHERE valid=1 ORDER BY id ASC');
	while ($proxy=$db->fetch($qproxy))
	{
		...
		$time=microtime(true);
		$cont=parseURLproxy( 'https://search.twitter.com/search.json?q=windows',$proxy['proxy'] );
		...
		$time=microtime(true);
		$cont=parseURLproxy( 'https://api.vkontakte.ru/method/newsfeed.search?q=путин',$proxy['proxy'] );
...
// 		print_r($proxy);
// 		$time=microtime(true);
// 		$cont=parseURLproxy( 'http:///tools/get_ok.php',$proxy['proxy'] );
// 		$ytime=(microtime(true)-$time);	
// 		echo $cont."\n";
// 		$mas=json_decode($cont,true);
// 		// print_r($mas);
