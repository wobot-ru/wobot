...
// echo 'SELECT * FROM blog_orders WHERE user_id!=145 AND third_sources!=0 and (order_end>='.mktime(0,0,0,date('n'),date('j')-1,date('Y')).') AND user_id!=0 AND ut_id!=0 ORDER BY order_id DESC';
// die();
$ressec=$db->query('SELECT * FROM blog_orders WHERE user_id!=145 AND third_sources!=0 and (order_end>='.mktime(0,0,0,date('n'),date('j')-1,date('Y')).') AND user_id!=0 AND ut_id!=0 ORDER BY order_id DESC');
