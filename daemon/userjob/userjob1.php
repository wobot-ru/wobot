...
$ressec=$db->query('SELECT * FROM robot_blogs2 WHERE '/*blog_last_update=0 OR blog_last_update<'.(time()-$deltatime*86400).' and*/.' (blog_link=\'facebook.com\')');
echo 'SELECT * FROM robot_blogs2 WHERE '/*blog_last_update=0 OR blog_last_update<'.(time()-$deltatime*86400).' and*/.' (blog_link=\'vkontakte.ru\' OR blog_link=\'facebook.com\')';
...
while($blog=$db->fetch($ressec))
{
	...
	echo $nick." ".$blog['blog_link']."\n";
	{
       ...
       elseif ($hn=='facebook.com') 
       {
           ...
           //echo $mas1[from][id];
           sleep(1);
           //$mas1[from][id]='1548406154';
           $mas['from']['id']=$blog['blog_login'];
           ...
           if (mysql_num_rows($tw)==0)
           {
               ...
               //$db->query('INSERT INTO robot_blogs2 (blog_link, blog_login,blog_location,blog_readers) values ("facebook.com","'.$mas1[from][id].'","'.$out_tw['loc_tw'][0].'",0)');
            	$upquery="UPDATE robot_blogs2 SET blog_link=".$hn.", blog_location='".$out_tw['loc_tw'][0]."',blog_readers=0 ,blog_last_update=".time().",blog_name='".$mas2['name']."' WHERE blog_login='".$nick."' AND blog_link='facebook.com'";
            	$db->query($upquery);
               echo 'gg add: '.$mas1[from][id].' '.($mas2[location][name])." |".$mas2['name']."| [facebook.com]\n";
            }
       }
       elseif ($hn=='vkontakte.ru') 
       {
           ...
           //print_r($resp);
           echo $resp['response'][0]['first_name']." ".$resp['response'][0]['last_name'];
           ...
           if (mysql_num_rows($tw)==0)
           {
                ...
                print_r($resp);
            	$upquery="UPDATE robot_blogs2 SET blog_link='".$hn."', blog_location='".$out_tw['loc_tw'][0]."',blog_readers=0 ,blog_last_update=".time().",blog_name='".$nicks."' WHERE blog_login='".$nick."' AND blog_link='vkontakte.ru'";
            	echo "UPDATE robot_blogs2 SET blog_link='".$hn."', blog_location='".$out_tw['loc_tw'][0]."',blog_readers=0 ,blog_last_update=".time().",blog_nick='".$nicks."' WHERE blog_login='".$nick."' AND blog_link='vkontakte.ru'";
            	$db->query($upquery);
                echo 'add: '.$out['vk_name'][0].' '.($name_cit)."|".$nicks."| [vkontakte.ru]\n";
           }
        	sleep(1);
		}
