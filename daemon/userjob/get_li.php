...
function get_liveinternet($nick) 
{
	global $db,$fem,$mail,$arrgen;
	do
	{
		$cont=parseUrlproxy('http://www.liveinternet.ru/users/'.$nick.'/profile');
		if ($cont=='')
		{
			$attmp++;
			echo "\n".'continue...'."\n";
	...
	while (($cont=='') && ($attmp<3));
	//$cont=parseUrl('http://www.liveinternet.ru/users/'.$nick.'/profile');
	$cont=iconv('windows-1251','UTF-8',$cont);
	$regex='/<span class\=[\"\']GlMarSmB[\"\']><b>Возраст\:<\/b>\s(?<age>\d+)\s[^<]*?<\/span>/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['age']=$out['age'][0];
	$regex='/<li class\=[\'\"]ProBlockMenuMore[\'\"]><a href=[\"\']http\:\/\/www\.liveinternet\.ru\/jsearch\/\?clist\=1[^<]*?>Все\s\((?<fol>\d+)\)<\/a><\/li>/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['fol']=$out['fol'][0];
	$regex='/<a href\=[\'\"]\/journal_city\.php\?cityid\=[^<]*?[\'\"]>(?<loc>[^<]*?)<\/a>/isu';
	preg_match_all($regex,$cont,$out);
	$loc=$out['loc'][0];
	if ($loc!='')
	{
	...
	$outmas['loc']=$twl;
	$outmas['nick']=$nick;
	$regex='/<h2 class=[\'\"]GlMarSmB[\'\"]><a[^<]*?>(?<name>[^<]*?)<\/a><\/h2>/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['name']=($out['name'][0]==''?$nick:$out['name'][0]);
	$regex='/http\:\/\/i\.li\.ru\/4Ek\/i\/all\/pen\/(?<gen>[\w\m])2\.gif/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['gender']=$arrgen[$out['gen'][0]];
	$regex='/<div class="ProTop-Vcard-AvaPicInner">\s*<a style=\'background-image:url\((?<ico>.*?)\)\;/isu';
	preg_match_all($regex, $cont, $out);
	//print_r($out);
