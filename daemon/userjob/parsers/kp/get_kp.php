...
function get_kp($nick) 
{
	global $db,$fem,$mail,$arrgen;
	do
	{
		$cont=parseUrlproxy('http://blog.kp.ru/users/'.$nick.'/profile');
		if ($cont=='')
		{
			$attmp++;
			echo "\n".'continue...'."\n";
	...
	$regex='/class="li-icons li-icons_(?<gen>[a-z]+)" title="" href=".*?">(?<nick>.*?)<\/a>/isu';
	preg_match_all($regex, $cont, $out);
	$outmas['nick']=$out['nick'][0];
	$outmas['name']=$out['nick'][0];
	$outmas['gender']=0;
	if ($out['gen'][0]=='Man') $outmas['gender']=2;
	if ($out['gen'][0]=='Woman') $outmas['gender']=1;
	$regex='/<b>Место проживания\:<\/b>\s+<a href="[^\"]+">(?<loc>.*?)<\/a>/isu';
	preg_match_all($regex, $cont, $out);
	$loc=$out['loc'][0];
	if ($loc!='')
	{
	...
	$regex='/<b>Возраст\:<\/b> (?<age>\d+) /isu';
	preg_match_all($regex, $cont, $out);
	$outmas['loc']=$twl;
	$outmas['age']=intval($out['age'][0]);
	$outmas['login']=$nick;
	$regex='/<a style=\'background-image\:url\((?<ico>.*?)\)\;\' href=\'javascript\:void\(0\)\;\'>/isu';
	preg_match_all($regex, $cont, $out);
