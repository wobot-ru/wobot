...
function get_foursquare($nick) 
{
	global $db,$fem,$mail,$arrgen;
	do
	{
		$cont=parseUrlproxy('https://ru.foursquare.com/'.(!preg_match('/[a-z]/isu',$nick)?'user/':'').$nick);
		if ($cont=='')
		{
			$attmp++;
			echo "\n".'continue...'."\n";
	...
	while (($cont=='') && ($attmp<3));
	//$cont=parseUrl('https://ru.foursquare.com/'.(!preg_match('/[a-z]/isu',$nick)?'user/':'').$nick);
	$regex='/<div id="userDetails">\s*<h\d>(?<name>.*?)<\/h\d>/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['name']=html_entity_decode($out['name'][0],ENT_QUOTES,'UTF-8');
	...
	// $regex='/<span class="translate">Друзья<\/span> <em>\((?<fol>\d+) total\)<\/em>/isu';
	$regex='/\"friends\"\:\{\"count\"\:(?<fol>\d+)\,/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['fol']=intval($out['fol'][0]);
	$regex='/<span class="homeTown">(?<loc>.*?)<\/span>/isu';
	preg_match_all($regex,$cont,$out);
	$loc=html_entity_decode($out['loc'][0],ENT_QUOTES,'UTF-8');
	if ($loc!='')
	{
	...
	$outmas['age']=0;
	$outmas['nick']=$nick;
	$regex='/<div id=\"userPic\"><img src\=\"(?<ico>[^\"]*?)\"/isu';
	preg_match_all($regex, $cont, $out);
