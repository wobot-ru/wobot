...
require_once('get_baby_other.php');
...
$i=0;
$ressec=$db->query('SELECT * FROM `blog_post` WHERE `post_link` LIKE \'%www.babyblog.ru/com%\' AND blog_id=0 ORDER BY post_id DESC');
...
while($blog=$db->fetch($ressec))
{
	...
	{
		$inf=get_baby_community($blog['post_link']);
		print_r($inf);
		if (($inf['nick']!='') && ($inf['name']!=''))
		{
			$iss=$db->query('SELECT * FROM robot_blogs2 WHERE blog_link=\'babyblog.ru\' AND blog_login=\''.$inf['nick'].'\'');
			if (mysql_num_rows($iss)==0)
			{
				echo 'INSERT INTO robot_blogs2 (blog_link,blog_login,blog_nick) VALUES (\'babyblog.ru\',\''.$inf['nick'].'\',\''.$inf['name'].'\')'."\n";
				$db->query('INSERT INTO robot_blogs2 (blog_link,blog_login,blog_nick) VALUES (\'babyblog.ru\',\''.$inf['nick'].'\',\''.$inf['name'].'\')');
