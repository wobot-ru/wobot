...
require_once('get_mail_community.php');
...
{
$i=0;
$ressec=$db->query('SELECT *  FROM `blog_post` WHERE `post_link` LIKE \'%mail.ru/community%\' AND blog_id=0 ORDER BY post_id DESC');
...
while($blog=$db->fetch($ressec))
{
	...
	{
		$inf=get_mail_community($blog['post_link']);
		print_r($inf);
		if (($inf['nick']!='') && ($inf['name']!='') && ($inf['type']!=''))
		{
			$iss=$db->query('SELECT * FROM robot_blogs2 WHERE blog_link=\'mail.ru/'.$inf['type'].'\' AND blog_login=\''.$inf['nick'].'\'');
			if (mysql_num_rows($iss)==0)
			{
				echo 'INSERT INTO robot_blogs2 (blog_link,blog_login,blog_nick) VALUES (\'mail.ru/'.$inf['type'].'\',\''.$inf['nick'].'\',\''.$inf['name'].'\')'."\n";
				$db->query('INSERT INTO robot_blogs2 (blog_link,blog_login,blog_nick) VALUES (\'mail.ru/'.$inf['type'].'\',\''.$inf['nick'].'\',\''.$inf['name'].'\')');
