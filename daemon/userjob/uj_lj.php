...
if (($gg1 % 2)==1)
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link FROM robot_blogs2 WHERE blog_last_update=0 AND blog_link=\'livejournal\.com\' ORDER BY blog_id DESC');
}
else
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link FROM robot_blogs2 WHERE blog_last_update<'.(time()-$deltatime*86400).' AND blog_link=\'livejournal\.com\' ORDER BY blog_last_update DESC LIMIT 100');
}
...
while($blog=$db->fetch($ressec))
{
	...
	if ($blog['blog_link']=='livejournal.com')
	{
		//echo $blog['blog_link'];
