...
if (($gg1 % 2)==1)
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link,blog_nick FROM robot_blogs2 WHERE blog_last_update=0 AND blog_link LIKE \'mail\.ru\/%\' ORDER BY blog_id DESC');
}
else
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link,blog_nick FROM robot_blogs2 WHERE blog_last_update<'.(time()-$deltatime*86400).' AND blog_link LIKE \'mail.ru/%\' ORDER BY blog_last_update DESC LIMIT 100');
}
