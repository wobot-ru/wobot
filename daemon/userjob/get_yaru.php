...
function get_yaru($nick) 
{
	global $db,$fem,$mail,$arrgen;
	do
	{
		$cont=parseUrlproxy('http://'.$nick.'.ya.ru/');
		if ($cont=='')
		{
			$attmp++;
			echo "\n".'continue...'."\n";
	...
	while (($cont=='') && ($attmp<3));	
	//$cont=parseUrl('http://'.$nick.'.ya.ru/');
	//echo $cont;
	$regex='/<div class\=\"b\-yaru\-widget\-user\-info\_\_age\"><i class\=\"b\-yaru\-widget\-user\-info\_\_sex b\-yaru\-widget\-user\-info\_\_sex\_type\_(?<gen>.*?)\"><\/i>\s(?<age>\d+)[^\/]*?<\/div>/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['age']=$out['age'][0];
	$outmas['gender']=$arrgen[$out['gen'][0]];
	$regex='/<a[^<]*?class="fn nickname">(?<nick>.*?)<\/a>/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['name']=$out['nick'][0];
	$outmas['nick']=$nick;
	$regex='/<div class="b-yaru-widget__title-link">\s*?<a class="b-link"[^<]*?>(?<fol>\d+)[^<]*?<\/a>/isu';
	preg_match_all($regex,$cont,$out);
	$outmas['fol']=$out['fol'][0];
	$regex='/<div class="b-yaru-widget-user-info__place">(?<loc>.*?)<\/div>/isu';
	preg_match_all($regex,$cont,$out);
	$loc=strip_tags($out['loc'][0]);
	if ($loc!='')
	{
	...
	$outmas['loc']=$twl;
	$regex='/<a class="b-userpic">\s*<img[^<]*?src=[\'\"](?<ico>.*?)[\'\"]/isu';
	preg_match_all($regex, $cont, $out);
	//print_r($out);
