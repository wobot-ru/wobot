...
if (($gg1 % 2)==0)
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link,blog_nick FROM robot_blogs2 WHERE blog_last_update=0 AND blog_link=\'twitter.com\' ORDER BY blog_id DESC');
}
else
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link,blog_nick FROM robot_blogs2 WHERE blog_last_update<'.(time()-$deltatime*86400).' AND blog_link=\'twitter.com\' ORDER BY blog_last_update DESC LIMIT 100');
}
...
while($blog=$db->fetch($ressec))
{
	...
	{
		//echo 'iter='.($iter % 6)."\n";
		$inf=get_twitter($blog['blog_login'],$iter % 6);
		$iter++;
		print_r($inf);
		//echo $blog['blog_nick'].' '.$blog['blog_login'];
		$qw='UPDATE robot_blogs2 SET blog_location=\''.$inf['loc'].'\', blog_readers='.intval($inf['fol']).', blog_nick=\''.$blog['blog_login'].'\', blog_login=\''.$blog['blog_login'].'\',blog_last_update=\''.time().'\' WHERE blog_id='.$blog['blog_id'];
		//echo $qw;
		$rru=$db->query($qw);
