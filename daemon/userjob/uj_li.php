...
if (($gg1 % 2)==1)
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link,blog_nick FROM robot_blogs2 WHERE blog_last_update=0 AND (blog_link=\'liveinternet\.ru\' OR blog_link=\'kp\.ru\' OR blog_link=\'aif\.ru\') ORDER BY blog_id DESC');
}
else
{
$ressec=$db->query('SELECT blog_id,blog_login,blog_link,blog_nick FROM robot_blogs2 WHERE blog_last_update<'.(time()-$deltatime*86400).' AND (blog_link=\'liveinternet\.ru\' OR blog_link=\'kp\.ru\' OR blog_link=\'aif\.ru\') ORDER BY blog_last_update DESC LIMIT 100');
}
...
while($blog=$db->fetch($ressec))
{
	...
	if (($blog['blog_link']=='liveinternet.ru') || ($blog['blog_link']=='kp.ru') || ($blog['blog_link']=='aif.ru'))
	{
