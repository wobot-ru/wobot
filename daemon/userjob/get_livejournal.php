...
function get_lj($nick)
{
	global $db,$fem,$mail;
	do
	{
		$cont=parseUrlproxy('http://'.$nick.'.livejournal.com/profile');
		if ($cont=='')
		{
			$attmp++;
			echo "\n".'continue...'."\n";
	...
	while (($cont=='') && ($attmp<3));
	//$cont=parseUrl('http://'.$nick.'.livejournal.com/profile');
	//echo $cont;
	$regex='/<a href=\'.*?\' class=\'region\'>(?<data>.*?)<\/a>/is';
	preg_match_all($regex,$cont,$outl);
	if ($outl['data'][0]=='')
	{
		$regex='/<a href=\'.*?\' class=\'((locality)|(country-name))\'>(?<data>.*?)<\/a>/is';
		preg_match_all($regex,$cont,$outl);
	}
	$loc=$outl['data'][0];
	/*$regex='/<span class=[\'"]expandcollapse on[\'"].*?><img id=[\'"]fofs_arrow[\'"].*?>.*?\((?<fol>.*?)\).<\/span>/is';*/
	//preg_match_all($regex,$cont,$out);
	//$out['fol'][0]=preg_replace('/[^0-9]/is','',$out['fol'][0]);
	$regex='/Site\.page \= (?<info>.*?);\s*Site\.page\.template/isu';
	preg_match_all($regex, $cont, $out);
	$json_info=json_decode(trim($out['info'][0]),true);
	$outmas['fol']=intval(preg_replace('/\s+/isu', '', $json_info['socconns'][0]['count']));
	// die();
	// $xpth_fol='//li[@data-action="friendof"]/a/span[@class="b-friends-count"]';
	// $out['fol'][0]=get_pst_al($xpth_fol,$cont);
	// $outmas['fol']=intval(preg_replace('/[^\d]/isu','',$out['fol'][0]));
	//echo $loc;
	if ($loc!='')
	{
	...
	$outmas['loc']=$twl;
	//$regex='/<tr><th>Name\:<\/th><td>(?<name>.*?)<\/td><\/tr>/isu';
	//preg_match_all($regex,$cont,$out);
	$xpth_name='//h1[@class="b-details-journal-title"]';
	$out['name'][0]=get_pst_al($xpth_name,$cont);
	...
	$outmas['gender']=$mm;
	$regex='/<img\s+src=\'(?<ico>[^<]*?)\'[^<]*?class=\'user_pic\'/isu';
	preg_match_all($regex, $cont, $out);
	$outmas['ico']=$out['ico'][0];
	$regex='/<dd class=\'b-profile-group-body\'>\s*\d\d?\s+[а-яёa-z]+\s+(?<y>\d\d\d\d)\s*<\/dd>/isu';
	preg_match_all($regex,$cont,$out5);
	//print_r($out5);
	//$xpth_birth='//dl[@class="b-profile-group b-profile-userinfo"]/dd[@class="b-profile-group-body"][2]';
	//$out5['byt'][0]=get_pst_al($xpth_birth,$cont);
	//echo $out5['byt'][0];
	//$rg='/(?<d>\d\d)\s+(?<m>[а-яёa-z]+)\s+(?<y>\d\d\d\d)/isu';
	//preg_match_all($rg,$out5['byt'][0],$ott);
	//print_r($ott);
	//$age1=0;
	//if (($ott['y'][0]!='') && ($ott['d'][0]!='') && ($ott['m'][0]!=''))
	//{
	//	$count++;
	//	$age1=date('Y')-$ott['y'][0];
	//}
	if (intval($out5['y'][0])!=0) $age1=date('Y')-intval($out5['y'][0]);
	$outmas['age']=intval($age1);
	$outmas['nick']=$nick;
	$outmas['name']=$nick;
	// print_r($outmas);
    //sleep(1);
