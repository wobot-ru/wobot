...
function get_mas_week($id,$start,$end)
{
	...
	$ressec=$db->query('SELECT * FROM blog_post WHERE order_id='.$id.' AND (post_time>='.($start).' AND post_time<'.($end+86400).') ORDER by post_time DESC');
	//echo 'SELECT * FROM blog_post WHERE order_id='.$id.' AND (post_time>'.$day.' AND post_time<'.($day+86400).')';
	//for ($i=0;$i<23;$i++)
	{
	//	$outmas['time'][$i]=0;
	...
	while($blog=$db->fetch($ressec))
	{
		...
		if ($blog['post_time']<($end-$k*86400*7))
		{
			//$outmas['time'][]
			$k++;
			$outmas['time'][$end-($k-1)*86400*7]=0;
		}
		else
		{
...
function get_mas_mon($id,$start,$end)
{
	...
	while($blog=$db->fetch($ressec))
	{
		...
		//echo $k.' '.($end-$k*86400*7).'<br>';
		if ($blog['post_time']<($end-$k*86400*30))
		{
			//$outmas['time'][]
			$k++;
			$outmas['time'][$end-$k*86400*30]=0;
		}
		else
		{
			$outmas['time'][$end-$k*86400*30]++;
		}
	}
	ksort($outmas['time']);
	//print_r($outmas);
