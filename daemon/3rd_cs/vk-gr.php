...
function get_vk_group($grid,$ts,$te)
{
	...
	$json=parseUrl('https://api.vkontakte.ru/method/groups.getById?gid='.urlencode($grid).'&access_token='.$access_token);
	$grinfo=json_decode($json,true);
	$gr_id=$grinfo['response'][0]['gid'];
	do
	{
		sleep(1);
		...
		foreach ($cont_wall['response']['wall'] as $key => $item)
		{
			if ($key>0)
			{
				if (($item['date']>$ts) && ($item['date']<$te))
				{
					...
					$adv_eng['repost']=intval($item['reposts']['count']);
...
//get_vk_group('tcsbank',mktime(0,0,0,12,1,2012),mktime(0,0,0,12,5,2012));
