...
function get_vk_board_topic($bid,$tid,$ts,$te)
{
	...
	do
	{
		echo '.';
		sleep(1);
		$cont=parseUrl('https://api.vkontakte.ru/method/board.getComments?gid='.$bid.'&tid='.$tid.'&access_token='.$access_token.'&count=100&offset='.intval($i*100));
		$mas=json_decode($cont,true);
		//print_r($mas);
		foreach ($mas['response']['comments'] as $key => $item)
		{
			if ($key==0) continue;
			if (($item['date']<$ts) || ($item['date']>($te+86400))) continue;
			$out['link'][]='http://vk.com/topic-'.$bid.'_'.$tid.'?post='.$item['id'];
			$out['time'][]=$item['date'];
	...
	while ((count($mas['response']['comments'])==101) && ($mas['response']['comments'][100]['date']<($te+86400)));
	print_r($out);
	return $out;
}

function get_vk_board($bid,$ts,$te)
{
	global $access_token,$redis;
	...
	{
		sleep(1);
		$cont=parseUrl('https://api.vkontakte.ru/method/board.getTopics?gid='.$bid.'&access_token='.$access_token.'&count=100&offset='.intval($i*100));
		$mas=json_decode($cont,true);
		foreach ($mas['response']['topics'] as $key => $item)
		{
			if ($key==0) continue;
			echo '/';
			$out=get_vk_board_topic($bid,$item['tid'],$ts,$te);
...
//get_vk_board(1,mktime(0,0,0,10,1,2012),mktime(0,0,0,11,30,2012));
