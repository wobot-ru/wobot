...
//echo 123;
$ressec=$db->query('SELECT a.tp_id,a.order_id,a.tp_last,a.gr_id,a.tp_type,b.order_start,b.order_end,b.order_keyword,a.tp_filter FROM blog_tp as a LEFT JOIN blog_orders as b ON a.order_id=b.order_id WHERE (a.tp_last<=b.order_end) AND a.tp_id='.$_SERVER['argv'][1].' ORDER BY tp_id DESC');
echo 'SELECT a.tp_id,a.order_id,a.tp_last,a.gr_id,a.tp_type,b.order_start,b.order_end,b.order_keyword,a.tp_filter FROM blog_tp as a LEFT JOIN blog_orders as b ON a.order_id=b.order_id WHERE (a.tp_last<=b.order_end) AND a.tp_id='.$_SERVER['argv'][1].' ORDER BY tp_id DESC';
...
while($blog=$db->fetch($ressec))
{
	...
	foreach ($m1['link'] as $key => $item)
	{
		...
		$qw=$db->query('SELECT post_id FROM blog_post WHERE post_content=\''.addslashes(mb_substr($m1['content'][$key],0,150,'UTF-8').' ...').'\' AND post_time='.$m1['time'][$key].' AND order_id='.$blog['order_id']);
...
mail('zmei123@yandex.ru','Сборщик 3rd_cs',$count_pst,$headers);
