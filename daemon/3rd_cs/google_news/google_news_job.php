...
$qmd5=$db->query('SELECT * FROM blog_tp WHERE order_id='.$order_id.' AND tp_type=\'google_news\'');
...
{
	$posts=get_google_news($item1['query']);
	print_r($posts);
	foreach ($posts['link'] as $key => $item)
	{
		if ($posts['time'][$key]>$order['order_end']+86400) continue;
		if ($posts['time'][$key]<$order['order_start']) continue;
	...
	$db->query('UPDATE blog_tp SET tp_last='.time().' WHERE tp_id='.$key1);
}
...
if (date('H')==0) mail('zmei123@yandex.ru','google_news',$order_id.' '.count($posts['time']),$headers);
