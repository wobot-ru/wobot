...
class users
{
	...
	public function get_vk()
	{
		...
		if (intval($acc_id['id_acc'][0])==0)
		{
			$rgx='/video(?<id_acc>\d+)/is';
			preg_match_all($rgx,$this->link,$acc_id);
		}
		if (intval($acc_id['id_acc'][0])==0)
		{
			$rgx='/note(?<id_acc>\d+)/is';
			preg_match_all($rgx,$this->link,$acc_id);
		}
		if (intval($acc_id['id_acc'][0])==0)
		{
			$rgx='/photo(?<id_acc>\d+)/is';
			preg_match_all($rgx,$this->link,$acc_id);
		}
		if (intval($acc_id['id_acc'][0])==0)
		{
	...
	public function get_facebook($mode=0)
	{
		global $db;
		if ($mode==0)
		{
	...
	public function get_kp()
	{
		...
		if ($acc_id['id_acc'][0]!='')
		{
			$chbb=$db->query('SELECT * from robot_blogs2 WHERE blog_login=\''.addslashes($acc_id['id_acc'][0]).'\' AND blog_link=\'kp\.ru\' LIMIT 1');
			if (mysql_num_rows($chbb)==0)
			{
				$ins_bb=$db->query('INSERT INTO robot_blogs2 (blog_link,blog_login) VALUES (\'kp\.ru\',\''.addslashes($acc_id['id_acc'][0]).'\')');
				$bb1['blog_id']=$db->insert_id();
			}
			else
			{
	...
	public function get_nick($url,$all_info=0)
	{
		$this->link=$url;
		$this->blog_id=0;
		$this->get_hn();
