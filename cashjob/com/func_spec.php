...
function get_cash($orderr,$time,$mode,$sub_settings)
{
	...
	if ($mode=='s') $addquery=get_subquery($sub_settings);
	$qpost=$db->query('SELECT p.post_link,p.post_content,p.post_nastr,p.post_engage,p.post_advengage,r.blog_id,r.blog_nick,r.blog_login,r.blog_readers,r.blog_location,p.post_note_count,p.post_read FROM blog_post as p LEFT JOIN robot_blogs2 as r ON p.blog_id=r.blog_id WHERE p.order_id='.$orderr['order_id'].' '.$addquery.' AND p.post_time>='.$stime.' AND p.post_time<'.$etime.(intval($settings['remove_spam'])==0?'':' AND p.post_spam!=1'));
	//echo 'SELECT p.post_link,p.post_content,p.post_nastr,p.post_engage,r.blog_id,r.blog_nick,r.blog_login,r.blog_readers,r.blog_location,p.post_note_count,p.post_read FROM blog_post as p LEFT JOIN robot_blogs2 as r ON p.blog_id=r.blog_id WHERE p.order_id='.$orderr['order_id'].' '.$addquery.' AND p.post_time>='.$stime.' AND p.post_time<'.$etime.(intval($settings['remove_spam'])==0?'':' AND p.post_spam!=1')."\n";
	while ($post=$db->fetch($qpost))
	{
		...
		if (($hn!='') && ($hn!='.'))
		{
			...
			if (isset($arr_nau[$hn]))
			{
				...
				$adv_engage=json_decode($post['adv_engage'],true);
				$beta_data['adv_engage']['retweet']+=$adv_engage['retweet'];
				$beta_data['adv_engage']['repost']+=$adv_engage['repost'];
				$beta_data['adv_engage']['comment']+=$adv_engage['comment'];
				$beta_data['adv_engage']['likes']+=$adv_engage['likes'];
	...
	//echo ' '.($mode=='s'?'sub':'').'order_'.($mode=='s'?$orderr['subtheme_id']:$orderr['order_id']).'_'.$stime.' ';
...
function getWords($text)
{
	...
	$words=preg_split('/([^а-яa-zё0-9]+)/isu', $text, null, PREG_SPLIT_NO_EMPTY);
	// $words=str_word_count($text,1,'абвгдеёжзиклмнопрстуфхцчшщъчьэюя');
	return $words;
}

function get_orders_cash($order_id,$mode)
{
	global $db,$redis;
	if ($mode!='s')
	{
	...
	{
		$qorder=$db->query('SELECT a.order_id,order_start,order_end FROM blog_orders as a LEFT JOIN blog_subthemes as b ON a.order_id=b.order_id WHERE b.subtheme_id='.$order_id.' LIMIT 1');
	}
	$order=$db->fetch($qorder);
	...
	{
		//echo ($mode=='s'?'sub':'').'order_'.$order_id.'_'.$t."\n";
		$var=$redis->get(($mode=='s'?'sub':'').'order_'.$order_id.'_'.$t);
	...
	//print_r($out);
	$redis->set(($mode=='s'?'sub':'').'orders_'.$order_id, json_encode($out));
	//print_r($din_post);
	//print_r($din_hn);
	//print_r($din_value);
	//print_r($out);
...
function get_main_img($mval,$order_id,$type)
{
	...
	$black = imagecolorallocate($imd,255,255,255);
	imagecolortransparent($imd, $black);
...
function get_subquery($params)
{
	//print_r($params);
	global $wobot;
	$query=' AND (';
	if ($params['tonal']['positive']==1) $query.=' p.post_nastr=1 ';
	$or=' OR ';
	if ($params['tonal']['negative']==1) $query.=$or.' p.post_nastr=-1 ';
	if ($params['tonal']['neutral']==1) $query.=$or.' p.post_nastr=0 ';
	if ($params['tonal']['undefined']==1) $query.=$or.' p.post_nastr=2 ';
	$query.=')';
	$or='';
	if (count($params['src'])!=0)
	{
		$query.=' AND (';
		foreach ($params['src'] as $item)
		{
			$query.=$or.' p.post_host=\''.$item.'\'';
			$or=' OR ';
		}
		$query.=')';
	}
	$or='';
	$and='';
	if (count($params['auth'])!=0)
	{
		$query.=' AND (';
		foreach ($params['auth'] as $item)
		{
			$query.=$or.' p.blog_id='.$item;
			$or=' OR ';
		}
		$query.=')';
	}
	$or='';
	$and='';
	if (count($params['loc'])!=0)
	{
		$query.=' AND (';
		foreach ($params['loc'] as $item)
		{
			if (isset($wobot['destn2'][$item]))
			{
				$query.=$or.' b.blog_location=\''.$wobot['destn2'][$item].'\'';
				$or=' OR ';
			}
		}
		$query.=')';
	...
	if (count($params['tags'])!=0)
	{
		...
		{
			$query.=$or.'(FIND_IN_SET(\''.$item.'\',post_tag)>0)';
			$or=' OR ';
		}
		$query.=')';
	}
