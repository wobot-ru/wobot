...
function run_crawling($gr_id)
{
	//$fp = fopen('/Applications/MAMP/htdocs/stgit/stools/st.log', 'a');
	$fp = fopen('/var/www/tpf/tpf.log', 'a');
	fwrite($fp, 'start: '.date('r').' crawl_'.$gr_id."\n");
	fclose($fp);
	...
	$process=proc_open('php /var/www/tpf/crawler.php '.intval($gr_id).' >> /var/www/tpf/logs/log_'.intval($gr_id).'.txt &',$descriptorspec,$pipes,$cwd,$end);
	/*$process=proc_open('php /var/www/tpf/crawler.php '.intval($gr_id).' &',$descriptorspec,$pipes,$cwd,$end);*//* or {
