...
$quser=$db->query('SELECT * FROM user_tariff as a LEFT JOIN blog_tariff as b ON a.tariff_id=b.tariff_id WHERE user_id='.$order['user_id'].' LIMIT 1');
$user=$db->fetch($quser);
$corder_info=$redis->get('orders_'.$order['order_id']);
$order_info=json_decode($corder_info,true);
// print_r($order_info);
$order_settings=json_decode($order['order_settings'],true);
// print_r($order_settings);
if ($order_settings['new_limit']=='' && $order_info['count_post']>$user['tariff_posts']) die('1');
if ($order_settings['new_limit']!='' && $order_info['count_post']>$order_settings['new_limit']) die('2');

// if (($order['user_id']==2039) || ($order['user_id']==2064) || ($order['user_id']==2403) || ($order['user_id']==145) || ($order['user_id']==3698)) die();
if ($order['user_id']==145) die();
// if ($order['user_id']==2039) die();
// if ($order['user_id']==2064) die();
...
$memcache->set('filters_'.$order['order_id'].'_'.$order['order_start'].'_'.($order['order_end']>time()?mktime(0,0,0,date('n'),date('j'),date('Y')):$order['order_end']), json_encode($out), MEMCACHE_COMPRESSED, 86400);
// print_r($out);
echo 'filters_'.$order['order_id'].'_'.$order['order_start'].'_'.($order['order_end']>time()?mktime(0,0,0,date('n'),date('j'),date('Y')):$order['order_end']);
