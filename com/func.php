...
function check_title($title,$mas)
{
	foreach ($mas as $item)
	{
		$regex='/.*?(?<word>'.$item.').*?/is';
		preg_match_all($regex,$title,$out);
		if ($out['word'][0]!='')
		{
			$c++;
		}
...
function get_main_img($mval,$order_id,$type)
{
	...
	chmod ( '/var/www/production/img/graph/'.$order_id.'_'.$type.'.png',0777);
}

function getcash($id,$start,$end)
{
	global $db,$wobot,$mwobot;
	$ressec=$db->query('SELECT * FROM blog_orders WHERE order_id='.$id);
	...
	while($blog=$db->fetch($ressec))
	{
		...
		if ((strtotime($start)==0) || (strtotime($end)==0))
		{
			...
			$sttt=(intval($blog['order_start'])-86400);
		}
		else
		{
			$enddd=strtotime($end);
			$sttt=strtotime($start);
			echo 'gg';
		}
		$query='SELECT * from blog_post as p LEFT JOIN robot_blogs2 AS b ON p.blog_id=b.blog_id WHERE p.order_id='.$blog['order_id'].' AND post_time>'.$sttt.' AND post_time<='.intval($enddd+86400);
	...
	echo '<br>Аудитория: '.$metrics['value'].'<br>';
	if (count($argv)==0)
	{
	if ($bleng==1)
	{
		echo '<br>Engagement: '.$eng;
	}
	echo '<br><br>Count posts: '.count($posts).'<br>';
	echo '<br>RESOURCES:</br>';
	arsort($socweb);
	foreach ($socweb as $key => $item)
	{
		echo $key.' '.$item.'<br>';
	}
	echo '<br>CITIES:<br>';
	...
	if (count($argv)==0)
	{
		/*echo '<script type="text/javascript">
			var word_list = [{text: "продам", weight: 608, url: "продам"},{text: "продаю", weight: 602, url: "продаю"},{text: "зима", weight: 334, url: "зима"},{text: "новый", weight: 254, url: "новый"},{text: "зимние", weight: 213, url: "зимние"},{text: "есть", weight: 160, url: "есть"},{text: "лето", weight: 151, url: "лето"},{text: "шипов", weight: 149, url: "шипов"},{text: "класс", weight: 118, url: "класс"},{text: "авто", weight: 117, url: "авто"},{text: "штампах", weight: 116, url: "штампах"},{text: "высший", weight: 112, url: "высший"},{text: "меня", weight: 104, url: "меня"},{text: "куплю", weight: 92, url: "куплю"},{text: "цены", weight: 91, url: "цены"},{text: "состоянии", weight: 82, url: "состоянии"},{text: "сезон", weight: 80, url: "сезон"},{text: "одной", weight: 80, url: "одной"},{text: "летней", weight: 79, url: "летней"},{text: "комплект", weight: 78, url: "комплект"},{text: "литье", weight: 75, url: "литье"},{text: "сборе", weight: 74, url: "сборе"},{text: "ниву", weight: 72, url: "ниву"},{text: "хорошая", weight: 66, url: "хорошая"},{text: "дверь", weight: 66, url: "дверь"},{text: "зимняя", weight: 63, url: "зимняя"},{text: "гараж", weight: 57, url: "гараж"},{text: "приду", weight: 56, url: "приду"},{text: "открою", weight: 55, url: "открою"},{text: "машины", weight: 54, url: "машины"},{text: "зимнюю", weight: 53, url: "зимнюю"},{text: "было", weight: 51, url: "было"},{text: "штамповке", weight: 47, url: "штамповке"},{text: "хищный", weight: 47, url: "хищный"},{text: "колёса", weight: 47, url: "колёса"},{text: "нету", weight: 46, url: "нету"},{text: "бока", weight: 42, url: "бока"},{text: "000р", weight: 40, url: "000р"},{text: "можно", weight: 38, url: "можно"},{text: "часть", weight: 37, url: "часть"},
			];
...
{
	global $argv;
	$masneed=array(1=>array("лига чемпионов","лиги чемпионов","лигой чемпионов"));
	$nword=array("здесь","всем","кто","что","сколько","какой","каков","чей","который","столько","этот","тот","такой","таков","весь","всякий","сам","самый","каждый","любой","другой","иной","никто","ничто","некого","нечего","нисколько","никакой","ничей","onclick","onclick=","return","style=","to=http%","fwww","href=","factions%","onmouseout=","textdecoration=","this","style","html","cursor","style","pointer","onmouseover=","underline","none","style=");
	foreach ($posts['words'] as $korder => $order)
	{
		$mas1=html_entity_decode($order,ENT_QUOTES);
		//echo $mas1."\n";
		$mas1=preg_replace('/<[\/]?br(\s[\/]?)?>/is',' ',$mas1);
		$mas1=strip_tags($mas1);
		$mas1=preg_replace('/\&nbsp\;/is',' ',$mas1);
		$mas1=preg_replace("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/is"," ",$mas1);
		foreach ($masneed as $item)
		{
			foreach ($item as $it)
			{
				//$mit=preg_replace('/(а|ев|ов|ие|ье|е|иями|ями|ами|еи|ии|и|ией|ей|ой|ий|ов|й|и|иям|ям|ием|ем|ам|ом|о|у|ах|иях|ях|ы|ь|ию|ью|ю|ия|ья|ь|я|еров)$/isu','',$mit);
				if (mb_strpos(mb_strtolower($mas1,"UTF-8"),$it,0,"UTF-8")!==false)
				{
					$masneed1[$item[0]]++;
				}
			}
		...
		$mas1=preg_replace('/\&lsquo\;/is',' ',$mas1);
		$mas1=preg_replace('/\&rsquo\;/is',' ',$mas1);
		$mas1=preg_replace('/\'/is',' ',$mas1);
		$mas1=preg_replace('/\#/is',' ',$mas1);
		...
		$mas1=preg_replace('/\>/isu',' ',$mas1);
		$mas1=preg_replace('/\?/isu',' ',$mas1);
		$mas1=preg_replace('/\&laquo\;/isu',' ',$mas1);
		$mas1=preg_replace('/\&raquo\;/isu',' ',$mas1);
		$mas1=preg_replace('/\"\;/isu',' ',$mas1);
		$mas1=preg_replace('/\&quot\;/isu',' ',$mas1);
		$mas1=preg_replace('/\«/isu',' ',$mas1);
		$mas1=preg_replace('/\»/isu',' ',$mas1);
		$mas1=str_replace('\n',' ',$mas1);
...
',' ',$mas1);
		...
		{
			$mas2=$item;
			$item=preg_replace('/(ая|а|ев|ов|ие|ье|е|иями|ями|ами|еи|ии|и|ией|ей|ой|ий|ов|й|и|иям|ям|ием|ем|ам|ом|о|у|ах|иях|ях|ы|ь|ию|ью|ю|ия|ья|ь|я|еров)$/isu','',$item);
			$item=preg_replace('/(ов|ы)$/isu','',$item);
			//echo $item."\n";
			if (!isset($c[$item]))
			{
				if (isset($outmas[$item]['reword']))
				{
					$c[$item]=1;
					$outmas[$item]['count']++;
					if ((!preg_match('/[^а-яА-Яa-zA-Z]/isu',$mas2)) && (!preg_match('/[^а-яА-Яa-zA-Z]/isu',$item)))
					{
						$outgigmas[mktime(0,0,0,date('n',$posts['time'][$korder]),date('j',$posts['time'][$korder]),date('Y',$posts['time'][$korder]))][preg_replace('/[^а-яА-Я]/isu','',$item)]['count']++;
						$outgigmas[mktime(0,0,0,date('n',$posts['time'][$korder]),date('j',$posts['time'][$korder]),date('Y',$posts['time'][$korder]))][preg_replace('/[^а-яА-Я]/isu','',$item)]['reword']=preg_replace('/[^а-яА-Я]/isu','',$mas2);
					}
				}	
				else
				{
	...
	foreach ($outmas as $key => $item)
	{
		...
		{
			if ((mb_strlen($item['reword'],"UTF-8")>3) && (!in_array($item['reword'],$nword)))
			{
				$i++;
	...
	foreach ($posts['words'] as $itt)
	{
		$op=0;
		foreach ($otmas['word'] as $itk)
		{
			$ww=$itk;
			$itk=preg_replace('/(ая|а|ев|ов|ие|ье|е|иями|ями|ами|еи|ии|и|ией|ей|ой|ий|ов|й|и|иям|ям|ием|ем|ам|ом|о|у|ах|иях|ях|ы|ь|ию|ью|ю|ия|ья|ь|я|еров)$/isu','',$itk);
			$itk=preg_replace('/(ов|ы)$/isu','',$itk);
			$op++;
			if (mb_strpos(mb_strtolower($itt,"UTF-8"),$itk,0,"UTF-8")!==false)
			{
				$ccc[$ww]++;
			}
		}
...
function gettopword($posts)
{
	$out=array("word" => array(), "col" => array());
	$gl=array('а','е','ё','и','о','у','ы','э','ю','я','й');
	foreach ($posts as $post)
	{
		$post=' '.$post;
		$post.='.';
		preg_match_all("/\s*(?<phrase>.*?)\s*[\.,!?\;]/is",$post,$off);
		$ou=$off["phrase"];
		//print_r($ou);
		foreach ($ou as $o)
		{
			...
			foreach ($offer as $of)
			{
				...
				if (part($of)==1)
				{
					while (in_array(mb_substr($of,mb_strlen($of,'UTF-8')-1,1,'UTF-8'),$gl))
					{
						$of=mb_substr($of,0,mb_strlen($of,'UTF-8')-1,'UTF-8');
					}
					//echo $of,"|";
					$ii=array_search($of,$out["word"]);
					if (($ii!==false) && ($of!=''))
					{
						$out["col"][$ii+1]++;
						print_r($out);
					...
					if (($ii!==true) && ($of!=''))	
					{
						$out["word"][count($out["word"])]=$of;
						$out["col"][count($out["word"])]=1;
						print_r($out);
						echo $of,"2<br>";
					}
				}
	...
	array_multisort($out["word"],SORT_DESC,$out["col"],SORT_DESC);
	return $out;
}

function part($word)
{
	$end=array("ая","яю","ой","ий","ий","ья","ье","ьи","ый","ая","ое","ые","яя","ее");
	$end_t=array("ому","ему");
	$c=0;
	//echo $word,"-";
	foreach ($end as $en)
	{
		//echo (mb_strpos($word,$en,0,"UTF-8"))," ";
		//echo "|",$word,"| |",$en,"|",mb_strpos($word,$en,0,"UTF-8")," ",(mb_strlen($word,"UTF-8")-2),"<br>";
		if (mb_strpos($word,$en,0,"UTF-8")==(mb_strlen($word,"UTF-8")-2))
		{
			$c=1;
			//echo 1;
			//echo $word;
	...
	foreach ($end_t as $en)
	{
		//mb_strpos($en,$word,"UTF-8")
		if (mb_strpos($word,$en,0,"UTF-8")==(mb_strlen($word,"UTF-8")-3))
		{
			$c=1;
			echo 1;
		}
...
{
	global $argv;
       $out=array("num" => array(), "nick" => array(), "posts" => array(), "id" => array());
       foreach ($posts as $post)
       {
               ...
               {
                       $out["num"][]=count($out["num"]);
                       $out["nick"][]=$post["nick"];
                       $out["posts"][]=1;
					   $out['site'][]=$post['site'];
					   $out['rnick'][]=$post['rnick'];
						$out['id'][]=$post['id'];
               }
       }
		//print_r($out);
       array_multisort($out["posts"],SORT_DESC,
       $out["nick"],SORT_DESC,$out["site"],SORT_DESC,$out["num"],SORT_DESC,$out["rnick"],SORT_DESC,$out["id"],SORT_DESC);
		unset($out["num"]);
		echo 'Количество уникальных авторов: '.count($out['nick']).'<br>';
		foreach ($out['posts'] as $key => $item)
		{
			if ($key<35)
			{
				$ott['posts'][]=$item;
				$ott['nick'][]=$out['nick'][$key];
				$ott['site'][]=$out['site'][$key];
				$ott['rnick'][]=$out['rnick'][$key];
				$ott['id'][]=$out['id'][$key];
			}
		}
		echo '<br>SPEAKERS:'."<br>";
		...
		if (count($argv)==0)
		{
		foreach ($ott['nick'] as $key => $item)
		{
			if ($ott['site'][$key]=='twitter.com')
			{
				echo '<a href="http://twitter.com/'.$ott['nick'][$key].'">http://twitter.com/'.$ott['nick'][$key	].'</a> '.$ott['posts'][$key]."<br>";
			}
			else
			if ($ott['site'][$key]=='livejournal.com')
			{
				echo '<a href="http://'.$ott['nick'][$key].'.livejournal.com/">http://'.$ott['nick'][$key].'.livejournal.com/</a> '.$ott['posts'][$key]."<br>";
			}
			else
			if ($ott['site'][$key]=='vkontakte.ru')
			{
				echo '<a href="http://vkontakte.ru/id'.$ott['nick'][$key].'">http://vkontakte.ru/id'.$ott['nick'][$key].'</a> '.$ott['posts'][$key]."<br>";
			}
			else
			if ($ott['site'][$key]=='facebook.ru')
			{
				echo '<a href="http://facebook.com/'.$ott['nick'][$key].'">http://facebook.com/'.$ott['nick'][$key].'</a> '.$ott['posts'][$key]."<br>";
			}
		}
	}
	$ott['uniq']=count($out['nick']);
	//print_r($ott);
       return $ott;
}
...
function promotion($posts)
{
	global $argv;
	//print_r($posts);
       $out=array("num" => array(), "nick" => array(), "posts" => array(), "readers" => array(), "site" => array(), "id" => array());
       foreach ($posts as $post)
       {
               ...
               //if ((!in_array($post["nick"],$out["nick"])) && ($post["nick"]!=""))
				if ((!isset($yetmas[$post["nick"].':'.$post["site"]])) && ($post["nick"]!=""))
               {
                       $out["num"][]=count($out["num"]);
                       $out["nick"][]=$post["nick"];
                       $out["posts"][]=1;
                       $out["readers"][]=$post["readers"];
					   $out['site'][]=$post["site"];
					   $out['rnick'][]=$post['rnick'];
					   $out['id'][]=$post['id'];
					$yetmas[$post["nick"].':'.$post["site"]]++;
               }
       }
//print_r($out);
       array_multisort($out["readers"],SORT_DESC,
       $out["nick"],SORT_DESC,$out["posts"],SORT_DESC,$out["site"],SORT_DESC,$out["num"],SORT_DESC,$out["rnick"],SORT_DESC,$out["id"],SORT_DESC);
		//print_r($out);
		unset($out["num"]);
		unset($ott["readers"]);
		unset($ott["nick"]);
		...
		unset($ott['id']);
		//print_r($out);
		foreach ($out['readers'] as $key => $item)
		{
			if ($key<35)
			{
				$ott['readers'][]=$item;
		...
		foreach ($ott['nick'] as $key => $item)
		{
			if ($ott['site'][$key]=='twitter.com')
			{
				echo '<a href="http://twitter.com/'.$ott['nick'][$key].'">http://twitter.com/'.$ott['nick'][$key	].'</a> '.$ott['readers'][$key]."<br>";
			}
			else
			if ($ott['site'][$key]=='livejournal.com')
			{
				echo '<a href="http://'.$ott['nick'][$key].'.livejournal.com/">http://'.$ott['nick'][$key].'.livejournal.com/</a> '.$ott['readers'][$key]."<br>";
			}
			else
			if ($ott['site'][$key]=='vkontakte.ru')
			{
				echo '<a href="http://vkontakte.ru/id'.$ott['nick'][$key].'">http://vkontakte.ru/id'.$ott['nick'][$key].'</a> '.$ott['readers'][$key]."<br>";
			}
			else
			if ($ott['site'][$key]=='facebook.ru')
			{
				echo '<a href="http://facebook.com/'.$ott['nick'][$key].'">http://facebook.com/'.$ott['nick'][$key].'</a> '.$ott['readers'][$key]."<br>";
			}
		}
	}
	//print_r($ott);
