...
while($blog=$db->fetch($ressec))
{
	...
	$query='SELECT * from blog_post as p LEFT JOIN robot_blogs2 AS b ON p.blog_id=b.blog_id WHERE p.order_id='.$blog['order_id'].' AND post_time>'.(intval($blog['order_start'])-86400).' AND post_time<='.intval($enddd);
	echo $query;
	...
	foreach ($posts as $gg => $post)
	{
			...
			if ($hn=='twitter.com') {
				...
				//if (mysql_num_rows($bman)>0)
				{
					...
					$allposts_prom[]=array('content'=>$content,'nick'=>$login1/*$nick*/,'readers'=>$post['reads'],'site'=>'twitter.com');
			...
			if (($hn=='livejournal.com') || ($hn=='vkontakte.ru') || ($hn=='facebook.com'))
			{
				...
				{
					//$man=$db->fetch($bman);
					$allposts[]=array('content'=>$content,'nick'=>$login/*$nick*/,'readers'=>$post['reads'],'site'=>$hn,'rnick'=>$nick);
					...
					//echo $login1.' '.$readers_prom."\n";
					$allposts_prom[]=array('content'=>$content,'nick'=>$login1/*$nick*/,'readers'=>$readers_prom,'site'=>$hn,'rnick'=>$login1);
					//echo $hn." ".$nick." ".$man['blog_readers']."\n";
					//array_push($allposts,array('content'=>$content,'nick'=>$man['blog_login'],'readers'=>$man['blog_readers'],'site'=>'twitter'));
				}
			}
...
//	fclose($fp);
	...
	$metrics['value']=value($allposts_prom);
