...
while($blog=$db->fetch($ressec))
{
	...
	foreach ($posts as $gg => $post)
	{
			...
			if ($hn=='twitter.com') {
				...
				//if (mysql_num_rows($bman)>0)
				{
					...
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['count']++;
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['readers']=$post['reads'];
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['login']=$nick;
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['nick']=$nick;
			...
			if (($hn=='livejournal.com') || ($hn=='vkontakte.ru') || ($hn=='facebook.com') || ($hn=='vk.com') || ($hn=='mail.ru') || ($hn=='liveinternet.ru') || ($hn=='ya.ru') || ($hn=='yandex.ru') || ($hn=='rutwit.ru') || ($hn=='rutvit.ru') || ($hn=='babyblog.ru') || ($hn=='blog.ru') || ($hn=='foursquare.com') || ($hn=='kp.ru') || ($hn=='aif.ru') || ($hn=='friendfeed.com'))
			{
				...
				//if (mysql_num_rows($bman)>0)
				{
					...
					//echo $nick.' '.$login1.' '.$hn."\n";
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['count']++;
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['readers']=$readers_prom;//$post['reads'];
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['login']=$login1;
					$beta_data['sp_pr'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$blog_id][$hn]['nick']=$nick;
					$beta_data['megamas'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$wobot['destn1'][$post['blog_location']]][$hn][$nastr]['count']++;
					$beta_data['megamas_wsp'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$wobot['destn1'][$post['blog_location']]][$hn][$nastr][$nick]['reads']=$post['reads'];
					$beta_data['megamas_wsp'][mktime(0,0,0,date('n',$time),date('j',$time),date('Y',$time))][$wobot['destn1'][$post['blog_location']]][$hn][$nastr][$nick]['count']++;
	...
	//print_r($yet_users1);
	//echo 123;
	print_r($beta_data['sp_pr'][1340568000]);
//print_r($loc);
//print_r($pcont);
//print_r($beta_data['sp_pr']);
//print_r($allposts_prom);
$topwords=gettopwords($pcont);
//print_r($topwords[1]);
$beta_data['words']=$topwords[1];
//print_r($beta_data['words']['1335816000']);
$topwords=$topwords[0];
//--------CLEARING WORD CLOUD-------
//---------"------------
$regex='/\"(?<words>[а-яА-Яa-zA-ZёЁ0-9\-]+)\"/isu';
preg_match_all($regex,mb_strtolower($blog['order_keyword'],'UTF-8'),$out);
$blog['order_keyword']=preg_replace('/\"(?<words>[а-яА-Яa-zA-ZёЁ0-9\-]+)\"/isu','',mb_strtolower($blog['order_keyword'],'UTF-8'));
foreach ($out['words'] as $item)
{
	foreach ($beta_data['words'] as $time => $itm)
	{
		if ($beta_data['words'][$time][$word->stem_word($item)]['reword']==$item)
		{
			unset($beta_data['words'][$time][$word->stem_word($item)]);
		}
	}
}
foreach ($out['words'] as $it)
{
	if (trim($it)=='') continue;
	unset($topwords[$it]);
}
//------------without "-----------
$keyword=preg_replace('/\~+[а-яА-Яa-zA-Z\-\ \.\'\"]+/isu','',mb_strtolower($blog['order_keyword'],'UTF-8'));
...
foreach ($keyword as $item)
{
	...
	foreach ($beta_data['words'] as $time => $itm)
	{
		//echo $time.' '.$word->stem_word($item)."\n";
		unset($beta_data['words'][$time][$word->stem_word($item)]);
		unset($beta_data['words'][$time][$item]);
		foreach ($itm as $k_it_wrd => $it_wrd)
		{
			//echo $it_wrd['reword'].' '.$item."\n";
			if (($it_wrd['reword']==$item) || ($it_wrd['reword']==$word->stem_word($item)))
			{
				//echo $item.'!!!!!!'.$k_it_wrd."\n";
				unset($beta_data['words'][$time][$k_it_wrd]);
			}
		}
	}
}
//die();
foreach ($keyword as $it)
{
	if (trim($it)=='') continue;
	unset($topwords[$it]);
	unset($topwords[$word->stem_word($it)]);
	foreach ($topwords as $key => $item)
	{
		if (($word->stem_word($it)=='') || ($word->stem_word($key)=='')) continue;
		//echo $word->stem_word($it).' '.$word->stem_word($key).' '.$it.' '.$key."\n";
		if (($word->stem_word($it)==$word->stem_word($key)) || (preg_match('/'.$word->stem_word($it).'[а-яА-Яa-zA-ZёЁ]*/isu',$key)) || (preg_match('/'.$word->stem_word($key).'[а-яА-Яa-zA-ZёЁ]*/isu',$it)))
		{
			//echo $key.'!!!'."\n";
			unset($topwords[$key]);
		}
...
//	fclose($fp);
	...
	if ($mval[count($mval)-2]!=0)
	{
		$metrics['d_post']=intval(100*($mval[count($mval)-1]-$mval[count($mval)-2])/$mval[count($mval)-2]);
	}
	else
	{
		if ($mval[count($mval)-1]==0)
		{
			$metrics['d_post']=0;
		}
		else
	...
	if (count($posts)>40000)
	{
		...
		$m->set('order_'.$blog['order_id'], json_encode($beta_data), MEMCACHE_COMPRESSED, 86400);
		//$m = new Memcached();
		//$m->addServer('localhost', 11211);
		//$m->setOption(Memcached::OPT_COMPRESSION, false);
		//$m->setMulti(array('order_'.$blog['order_id']=>json_encode($beta_data)),time()+86400);
		//$m->set('order_'.$blog['order_id'],$beta_data,time()+86400);
