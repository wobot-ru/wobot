...
if ($_GET['order_id']!='')
{
	...
	fwrite($fp, 're_charge start: '.date('r').' '.intval($_GET['order_id'])."\n");
	fclose($fp);
	...
	$cwd='/var/www/bot/';
	$end=array();
	echo 'php /var/www/daemon/fsearch3/re_charge.php '.intval($_GET['order_id']).' &';
	$process=proc_open('php /var/www/daemon/fsearch3/re_charge.php '.intval($_GET['order_id']).' &',$descriptorspec,$pipes,$cwd,$end);/* or {
