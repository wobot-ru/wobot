...
function embedly_embed_thumbnails(&$feed) {
	...
	foreach ($justUrls as $index => $url) {
		if ($thumb = $oembeds[$index]->thumbnail_url) {
			$html = theme('external_link', urldecode($url), "<img src='" . IMAGE_PROXY_URL . "x50/200/" . $thumb . "' />");
