...
function tokenize($line) {
	...
	while (strlen($line)) {
		$line = trim($line);
		// echo $line."\n";
		if (preg_match('/^[а-яa-zё\s]+(\/\s*\{?[\+\-]\d+\s*[\+\-]\d+\}?|\/[\-\+]?\d+)/isu',$line,$regs))
		{
			// print_r($regs);
...
// if (($_POST['post']!='')&&($_POST['kw']!=''))
// {
// 	echo check_post(urldecode($_POST['post']),urldecode($_POST['kw']));
// }
...
{
	$i++;
	$text='philips&o\'neill';
	$kw='"Tested on animals"|"tested on animal"|philips&o\'neill|philips&oneill|philips&oniell|philips&oneil|philips&o\'neil|филипс&онил|"неубиваемые наушники"|"наушники для экстрима"|stretch&наушники|specked&наушники|bend&наушники|tread&наушники|construct&наушники|toaheadphones';
	// echo urlencode($kw).' ';
	// echo urlencode($text);
...
// 	curl_setopt($curl, CURLOPT_URL, 'http://ec2-79-125-49-85.eu-west-1.compute.amazonaws.com/tools/epikur/infix.php');
...
// 	curl_setopt($curl, CURLOPT_POSTFIELDS, 'post='.urlencode($text).'&kw='.urlencode($kw));
