...
foreach ($mpost as $item)
{
	if (($item['post']!='')&&($item['link']!='')&&($item['date'].' '.$item['time']!=' '))
	{
		...
		if ($db->num_rows($qisset)==0)
		{
			...
			$qw='INSERT INTO blog_post (order_id,post_link,post_host,post_content,post_time,blog_id) VALUES ('.$_POST['orid'].',\''.addslashes($item['link']).'\',\''.$hn.'\',\''.addslashes($item['post']).'\','.(strtotime($item['date'].' '.$item['time'])+7200).','.$blog_id.')';
			// echo '<br>'.$qw.'<br>';
			$qinsert=$db->query($qw);
