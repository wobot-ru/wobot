...
$users=$db->query('SELECT post_id,post_link FROM blog_post WHERE blog_id=0 AND post_link LIKE \'http\:\/\/www\.liveinternet\.ru\/users\/%\'');
while ($user=$db->fetch($users))
{
	$regex='/http\:\/\/www\.liveinternet\.ru\/users\/(?<user>[^\/]*?)\//isu';
...
{
	$qw=$db->query('SELECT * FROM robot_blogs2 WHERE blog_link=\'liveinternet.ru\' AND blog_login=\''.$item['user'].'\' LIMIT 1');
	if (mysql_num_rows($qw)==0)
	{
		echo 'INSERT INTO robot_blogs2 (blog_link,blog_login) VALUES (\'mail.ru/'.$item['type'].'\',\''.$item['user'].'\')'."\n";
		$qins=$db->query('INSERT INTO robot_blogs2 (blog_link,blog_login) VALUES (\'liveinternet.ru\',\''.$item['user'].'\')');
		$idus=$db->insert_id();
	}
	else
	{
